...
### Requirement: 単一紐づけ — 選択した type の FK のみ保持し他をクリアする

createActionItem usecase and updateActionItem usecase SHALL enforce that at most one of `dealId`, `inquiryId`, `meetingId` is non-null. When a type is selected, the other two FK fields SHALL be set to null.

#### Scenario: 案件を選択すると inquiryId と meetingId が null になる

**Given** タスク作成時に dealId="D1" が指定される
**When** createActionItem が呼ばれる
**Then** 作成された ActionItem の dealId="D1"、inquiryId=null、meetingId=null である

#### Scenario: 編集で紐づけ先を案件から引合に変更する

**Given** ActionItem に dealId="D1", inquiryId=null, meetingId=null が設定されている
**When** updateActionItem で inquiryId="I1" が指定される
**Then** 更新後の ActionItem の dealId=null、inquiryId="I1"、meetingId=null である

#### Scenario: 紐づけを解除する（なし）

**Given** ActionItem に dealId="D1" が設定されている
**When** updateActionItem で dealId=null, inquiryId=null, meetingId=null が指定される
**Then** 更新後の ActionItem の 3 FK すべてが null である
