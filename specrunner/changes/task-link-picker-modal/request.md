...
2. **検索 Server Action**: `searchLinkTargetsAction({ type: "deal"|"inquiry"|"meeting", query })` を新設し、候補 `{ id, label }[]` を返す。label は 案件＝title、引合＝title、会議＝`${formatDateJP(date)} ${種別ラベル}`（親の案件/引合名があれば併記）。認証・テナント分離必須
...
4. **単一紐づけのセマンティクス**: タスクは案件・引合・会議のいずれか1つにのみ紐づく。ピッカーで選んだ type の FK をセットし、他2つの FK は null にする。createActionItem / updateActionItem の usecase でこの不変条件（3 FK のうち最大1つが非 null）を保証する
...
- [ ] タスク作成: ピッカーで選んだ単一紐づけが保存され、他2つの FK が null である
...
1. **単一紐づけに統一、複数 FK 同時セットを却下** — 「紐づけ先を1つ選ぶ」UI が明快で、会議は案件/引合に属するため親も辿れる。却下案: 現状の独立した複数 FK 維持 — UI が煩雑で「案件と引合の両方に紐づく」意味が曖昧
