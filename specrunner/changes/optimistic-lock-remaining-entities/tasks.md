...
- [x] `drizzle/0010_remaining_entity_version.sql` を作成する
...
- [x] `drizzle/meta/_journal.json` に idx=9, tag=`0010_remaining_entity_version` のエントリを追加する
- [x] `drizzle/meta/0009_snapshot.json` を作成する（`drizzle/meta/0008_snapshot.json` をベースに、meetings / action_items / revenue_targets 各テーブル定義の columns に version カラムエントリを追加した内容にする）
- [x] `src/infrastructure/schema.ts` の `meetings` テーブル定義に `version: integer("version").notNull().default(1)` を追加する（`updatedAt` の後）
- [x] `src/infrastructure/schema.ts` の `actionItems` テーブル定義に `version: integer("version").notNull().default(1)` を追加する（`updatedAt` の後）
- [x] `src/infrastructure/schema.ts` の `revenueTargets` テーブル定義に `version: integer("version").notNull().default(1)` を追加する（`updatedAt` の後）
...
- [x] `src/domain/models/meeting.ts` の `Meeting` 型に `version: number` フィールドを追加する（`updatedAt: Date` の後）
- [x] `src/domain/models/actionItem.ts` の `ActionItem` 型に `version: number` フィールドを追加する（`updatedAt: Date` の後）
- [x] `src/domain/models/revenueTarget.ts` の `RevenueTarget` 型に `version: number` フィールドを追加する（`updatedAt: Date` の後）
...
- [x] `src/infrastructure/repositories/meetingRepository.ts` の `mapRow` 関数の返却オブジェクトに `version: row.version` を追加する
- [x] `src/infrastructure/repositories/actionItemRepository.ts` の `mapRow` 関数の返却オブジェクトに `version: row.version` を追加する
- [x] `src/infrastructure/repositories/revenueTargetRepository.ts` の `mapRow` 関数の返却オブジェクトに `version: row.version` を追加する
...
- [x] `src/infrastructure/repositories/meetingRepository.ts` の `update` 関数:
...
- [x] `src/infrastructure/repositories/actionItemRepository.ts` の `update` 関数:
  - シグネチャに `expectedVersion: number` パラメータを追加する（`data` の後、`tx` の前）
  - `drizzle-orm` から `sql` をインポートする（既存の import 行に追加）
  - `.set()` 内に `version: sql\`version + 1\`` を追加する
  - `.where()` 条件に `eq(actionItems.version, expectedVersion)` を追加する
- [x] `src/infrastructure/repositories/revenueTargetRepository.ts` の `update` 関数:
  - シグネチャに `expectedVersion: number` パラメータを追加する（`data` の後、`tx` の前）
  - `drizzle-orm` から `sql` をインポートする（既存の import 行に追加）
  - `.set()` 内に `version: sql\`version + 1\`` を追加する
...
- [x] `src/application/usecases/updateMeeting.ts` を修正:
...
- [x] `src/application/usecases/updateActionItem.ts` を修正:
...
- [x] `src/application/usecases/toggleActionItemDone.ts` を修正:
...
- [x] `src/application/usecases/updateRevenueTarget.ts` を修正:
...
- [x] **Schema: version columns** セクションに meetings / action_items / revenue_targets の version カラム存在テストを追加する
  - `meetings` テーブルに `integer("version")`, `.notNull()`, `.default(1)` が存在する
  - `action_items` テーブルに同上
  - `revenue_targets` テーブルに同上
- [x] **Domain model: version field** セクションに Meeting / ActionItem / RevenueTarget の version フィールド存在テストを追加する
...
- [x] **Repository: optimistic lock WHERE clause** セクションに 3 リポジトリのテストを追加する
...
- [x] **Usecase: version passed to repository** セクションに 4 usecase のテストを追加する
...
- [x] **Usecase: optimistic lock failure message** セクションに 4 usecase のテストを追加する
