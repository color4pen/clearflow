...
- [ ] `src/app/(dashboard)/dashboard/DashboardHeader.tsx` を新規作成する
- [ ] Props: `title: string`, `subtitle: string`, `actions: ReactNode`
- [ ] レイアウト: flex、左に `title`（`text-lg font-bold`）+ `subtitle`（`text-xs text-text-muted`）を縦積み、右に `actions` スロット
- [ ] `"use client"` は不要（children を受け取るだけの表示コンポーネント）
...
- [ ] `SalesDashboard.tsx` から `PageToolbar` の import と使用を除去する
- [ ] `DashboardHeader` を導入し、title=「ダッシュボード」、subtitle に今日の日付を表示する
- [ ] actions スロットに 2 つのボタンを配置する:
...
- [ ] パイプラインサマリを 5 カラムから 6 カラム（`grid-cols-6`）に変更する
- [ ] 6 列目に「合計」セルを追加する。件数は `pipelineSummary.reduce((s, i) => s + i.count, 0)`、金額は同様に `totalAmount` を sum
- [ ] 合計セルはクリック時 `/deals` へ遷移（フェーズフィルタなし）
- [ ] 各セルの件数表示を大きめフォント（`text-xl font-bold`）+ 「件」サフィックスに変更する
- [ ] 各セルの金額表示に `font-mono` クラスを付与する
- [ ] 各セルに右 border（`border-r border-border`、最終セルは除外）で区切りを入れる
- [ ] `SectionCard` でラップする（既存通り）
...
- [ ] パイプラインサマリ以降のコンテンツを 2 カラム CSS grid に変更する
- [ ] グリッド定義: `grid` + `grid-cols-[1.55fr_1fr]` + `gap-6`（24px）
- [ ] 左カラム: アクション待ちリスト（SectionCard）
- [ ] 右カラム: 停滞案件（SectionCard）+ 直近の活動（SectionCard）を `space-y-2` で縦積み
- [ ] `staleDeals` が `null` の場合（非 manager/admin）、右カラムには直近の活動のみ表示する
...
- [ ] `<table>` を `<div>` ベースの flex レイアウトに置換する
- [ ] セクションヘッダーに件数表示を追加する（例: `12件`）
- [ ] 超過件数がある場合、ヘッダーに赤バッジ（`bg-danger text-white text-2xs rounded-full px-1.5`）で超過件数を表示する
- [ ] 超過判定: `approval` は `deadline` が過去、`action_item` は `dueDate` が過去。`inquiry` は超過なし
- [ ] 各行を flex で構成する:
...
- [ ] 各行に `border-b border-border-light` + `py-2` を付与する
- [ ] 空の場合は「アクション待ちのアイテムはありません」を表示する（既存メッセージ維持）
...
- [ ] 停滞日数の算出: `Math.floor((Date.now() - deal.updatedAt.getTime()) / 86400000)`
- [ ] `deal.assigneeName` が null の場合は担当者部分を省略する
- [ ] `deal.estimatedAmount` が null の場合は金額部分を省略する
- [ ] 各行に `border-b border-border-light` + `py-2` を付与する
- [ ] 空の場合は「停滞している案件はありません」を表示する（既存メッセージ維持）
...
- [ ] `<table>` を `<div>` ベースの flex レイアウトに置換する
- [ ] `formatRelativeTime(date: Date): string` ユーティリティ関数を `SalesDashboard.tsx` 内に追加する:
  - 1 分未満 → 「たった今」
  - 60 分未満 → 「○分前」
  - 24 時間未満 → 「○時間前」
  - それ以上 → 「○日前」
...
- [ ] 各行に `border-b border-border-light` + `py-1.5` を付与する
- [ ] 空の場合は「活動ログがありません」を表示する（既存メッセージ維持）
...
- [ ] `FinanceDashboard.tsx` から `PageToolbar` の import と使用を除去する
- [ ] `DashboardHeader` を導入し、title=「ダッシュボード（経理）」、subtitle に今日の日付を表示する
- [ ] actions スロットに「契約を見る」ボタン: outline スタイル（`border border-border text-text`）、`/contracts` へ `Link`
- [ ] 「今月の売上サマリ」の SectionCard を 4 カラム KPI グリッドに置換する
- [ ] グリッド定義: `grid` + `grid-cols-[1.4fr_1fr_1fr_1fr]` + `gap-3`
- [ ] 各 KPI セル（SectionCard 内 div）:
...
- [ ] KPI グリッド全体を SectionCard でラップする
- [ ] 各セルの値に `font-mono` を付与する
...
- [ ] 期日超過セクションの `InvoiceTable` 呼び出しを、専用の grid レイアウトに置換する
- [ ] ヘッダー行: 5 カラムグリッド（`grid-cols-[1fr_80px_100px_100px_80px]`）。列: 請求タイトル, 契約, 金額, 支払期日, 超過日数
  - ※ D7 決定に基づき、Invoice モデルに contractName / customerName がないため、「請求タイトル」+「契約リンク」で構成する
- [ ] ヘッダー行: `text-table-head text-text-muted bg-bg-table-head py-1.5 px-2`
- [ ] データ行: 各フィールドを grid セルとして表示する
...
- [ ] 超過日数の算出: `Math.floor((Date.now() - inv.dueDate.getTime()) / 86400000)`
- [ ] 各行に `border-b border-border-light py-1.5 px-2` を付与する
- [ ] 空の場合は「期日超過の請求はありません」を表示する
...
- [ ] 未入金セクションと請求予定セクションを 2 カラム CSS grid でラップする
- [ ] グリッド定義: `grid` + `grid-cols-2` + `gap-4`
- [ ] 左カラム: 「未入金の請求」（SectionCard）
- [ ] 右カラム: 「請求予定」（SectionCard）
- [ ] 両セクションの `InvoiceTable` / `UpcomingInvoiceTable` を flex レイアウトに置換する
- [ ] 各行を flex で構成する:
...
- [ ] 各行に `border-b border-border-light py-2` を付与する
- [ ] 各カラムのヘッダーに件数を表示する（既存の `{n}件` 表示を維持）
- [ ] 空の場合は既存メッセージを表示する
...
- [ ] `SalesDashboard.tsx` から未使用の import を除去する（`PageToolbar` 等）
- [ ] `FinanceDashboard.tsx` から未使用の import / コンポーネント（`InvoiceTable`, `UpcomingInvoiceTable`）を除去する
- [ ] `bun run build` が green であることを確認する
- [ ] 既存テストが pass することを確認する（`bun test` 等）
