...
- [ ] `src/app/(dashboard)/settings/SettingsNav.tsx` の `NAV_ITEMS` 配列の順序を変更する
...
- [ ] `src/app/(dashboard)/settings/policies/page.tsx` のカラム構成を確認
...
- [ ] admin 用の「操作」カラム（編集リンク + 有効化/無効化ボタン）はデザインに明示記載はないが、管理操作として必要なため維持する
...
- [ ] `src/app/(dashboard)/settings/templates/page.tsx` の columns 配列から「フィールド数」カラムを削除する
  - 削除対象: `{ key: "fields", header: "フィールド数", ... }`
- [ ] 残りの 3 カラム: テンプレート名, ステップ数, 作成日時
- [ ] 「作成日時」のヘッダーテキストを「作成日」に変更する
- [ ] admin 用の「操作」カラム（編集 + 削除）は維持する
...
- [ ] `src/app/(dashboard)/settings/users/page.tsx` の columns 配列から「作成日時」カラムを削除する
  - 削除対象: `{ key: "createdAt", header: "作成日時", ... }`
- [ ] 残りの 3 カラム: 名前, メールアドレス, ロール
...
- [ ] `src/app/(dashboard)/settings/delegations/page.tsx` を 2 セクションに分割する
  - セクション 1: 「自分の委任」 — `delegations` を `session.user.id` で `fromUserId` がマッチするものにフィルタ
  - セクション 2: 「全ユーザーの委任」 — admin 向けに全委任を表示（現在の表示と同じ）
- [ ] 各セクションに PageToolbar 相当のサブヘッダーを付ける（SectionCard のヘッダー部分を利用）
- [ ] 委任追加フォームは現状維持（スコープ外）
- [ ] データ取得ロジックは変更しない（既存の `listDelegationsAction` をそのまま使用）
...
- [ ] `src/app/(dashboard)/settings/webhooks/page.tsx` のカラムから「Secret」と「作成日時」を削除する
- [ ] 「直近配信状態」カラムを追加する
  - 配信ステータスの表示: 成功（text-success）/ 失敗（text-danger）/ 処理中（text-warning）/ 配信なし（text-text-disabled）
- [ ] カラム構成: URL, イベント数, 有効/無効, 直近配信状態 + 操作カラム
...
- [ ] `src/infrastructure/repositories/webhookDeliveryRepository.ts` に `findLatestByEndpointIds(endpointIds: string[], organizationId: string)` メソッドを追加する
  - エンドポイント ID リストを受け取り、各エンドポイントの最新 delivery の status を返す
  - 返り値: `Map<string, { status: WebhookDeliveryStatus; lastAttemptAt: Date | null }>`
- [ ] `src/app/actions/webhooks.ts` の `listWebhookEndpointsAction` を修正して、各 endpoint に `lastDeliveryStatus` を付与する
...
- [ ] `src/infrastructure/repositories/auditLogRepository.ts` の `findByOrganization` の `options` に `actorId?: string` と `targetType?: string` を追加する
  - 既存の `action` フィルタと同じパターンで `eq` 条件を追加する
- [ ] `src/app/(dashboard)/settings/audit-logs/page.tsx` のフィルタ UI に以下を追加する
...
- [ ] フィルタを 4 列グリッドに変更（操作者, 操作種別, 対象種別, 期間）
  - 期間は開始日と終了日を 1 セルにまとめるか、5 列に拡張する
- [ ] CSV エクスポート URL に新しいフィルタパラメータ（`actorId`, `targetType`）を含める
- [ ] `src/app/api/audit-logs/export/route.ts` でも `actorId` と `targetType` フィルタをサポートする
...
- [ ] `src/app/(dashboard)/settings/audit-logs/page.tsx` のカラム構成を調整する
