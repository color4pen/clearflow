...
- [x] `src/infrastructure/schema.ts` に `revenueTargets` テーブル定義を追加する:
...
- [x] `revenueTargetsRelations` を追加する（`organization` への `one` リレーション）
- [x] `organizationsRelations` に `revenueTargets: many(revenueTargets)` を追加する
...
- [x] `src/domain/models/index.ts` に `RevenueTarget`, `MonthlyRevenue`, `CustomerRevenue`, `DealRevenue`, `PipelineSummary` の export を追加する
...
- [x] `src/domain/authorization.ts` の `Entity` 型に `"revenue"` を追加する
- [x] `PERMISSION_MATRIX` に `revenue` エントリを追加する:
  - `view`: 全ロール（`ALL_ROLES`）
...
- [x] `src/infrastructure/repositories/revenueRepository.ts` を新規作成する
- [x] `getMonthlyRevenue(organizationId: string, startDate: Date, endDate: Date): Promise<MonthlyRevenue[]>` を実装する:
...
- [x] `getCustomerRevenue(organizationId: string, startDate: Date, endDate: Date, limit?: number): Promise<CustomerRevenue[]>` を実装する:
...
- [x] `getDealRevenue(organizationId: string, startDate: Date, endDate: Date): Promise<DealRevenue[]>` を実装する:
  - `invoices` JOIN `contracts` JOIN `deals` で集計
  - `status = "paid"` AND `paidAt` 期間フィルタ
  - `deals.id`, `deals.title` で GROUP BY し、SUM(amount), COUNT(*) を取得
  - amount DESC でソート
- [x] `getPipelineSummary(organizationId: string): Promise<PipelineSummary[]>` を実装する:
...
- [x] `src/infrastructure/repositories/index.ts` に `export * as revenueRepository from "./revenueRepository"` を追加する
...
- [x] `src/infrastructure/repositories/revenueTargetRepository.ts` を新規作成する
- [x] `mapRow` 関数を実装する（DB row → `RevenueTarget` ドメインモデル）
- [x] `create(data: { organizationId, periodStart, periodEnd, targetAmount }, tx?)` を実装する
- [x] `findById(id, organizationId, tx?)` を実装する
- [x] `findByOrganization(organizationId)` を実装する — 全目標を `periodStart` ASC でソート
- [x] `findByPeriod(organizationId, date: Date, tx?)` を実装する — 指定日が `periodStart <= date <= periodEnd` に含まれる目標を返す
- [x] `findOverlapping(organizationId, periodStart, periodEnd, excludeId?, tx?)` を実装する — 期間が重複する目標を検索する（`NOT (period_end <= startDate OR period_start >= endDate)` 条件）
- [x] `update(id, organizationId, data: Partial<{ periodStart, periodEnd, targetAmount }>, tx?)` を実装する
- [x] `deleteById(id, organizationId, tx?)` を実装する
- [x] `src/infrastructure/repositories/index.ts` に `export * as revenueTargetRepository from "./revenueTargetRepository"` を追加する
...
- [x] `src/application/usecases/getRevenueDashboard.ts` を新規作成する:
  - 引数: `{ organizationId }`
  ...
  - 全結果をまとめて返す
- [x] `src/application/usecases/getRevenueDetails.ts` を新規作成する:
...
- [x] `src/application/usecases/setRevenueTarget.ts` を新規作成する:
  ...
  - `{ ok: true, target }` を返す
- [x] `src/application/usecases/updateRevenueTarget.ts` を新規作成する:
...
- [x] `src/application/usecases/deleteRevenueTarget.ts` を新規作成する:
  - 引数: `{ id, organizationId }`
  - `revenueTargetRepository.deleteById` で削除する
- [x] `src/application/usecases/getRevenueForecast.ts` を新規作成する:
...
- [x] `src/app/actions/revenue.ts` を新規作成する
- [x] `setRevenueTargetAction(formData: FormData)` を実装する:
  - `auth()` で認証チェック
...
- [x] `updateRevenueTargetAction(formData: FormData)` を実装する:
  - 認証・認可・レート制限チェック
  - Zod スキーマで `id`（UUID）、`periodStart?`、`periodEnd?`、`targetAmount?` をバリデーション
  - `updateRevenueTarget` usecase を呼び出す
  - `revalidatePath("/revenue/forecast")` で再検証
- [x] `deleteRevenueTargetAction(id: string)` を実装する:
  - 認証・認可チェック
...
- [x] `src/app/(dashboard)/revenue/page.tsx` を新規作成する
- [x] `auth()` でセッションを取得し、`getRevenueDashboard` usecase を呼び出す
- [x] 今月の入金確認済み合計金額を `SectionCard` で表示する（`¥` + `toLocaleString("ja-JP")` フォーマット）
- [x] 月次推移データを `SectionCard` 内のテーブルまたはリストで表示する（12 ヶ月分）
- [x] パイプライン売上予測を `SectionCard` で表示する（フェーズ名、案件数、合計金額）
  - フェーズ名は `labels.ts` の `phaseLabels` を使用する
- [x] 顧客別売上ランキング（上位 10 社）を `SectionCard` + `DataTable` で表示する
- [x] `PageToolbar` で「売上ダッシュボード」タイトルを表示する
...
- [x] `src/app/(dashboard)/revenue/details/page.tsx` を新規作成する
- [x] URL の searchParams から `startDate`、`endDate`、`axis` パラメータを取得する
- [x] デフォルト値: `startDate` = 当月初日、`endDate` = 当月末日、`axis` = `"monthly"`
- [x] フィルタ UI を実装する — 開始日・終了日の入力と集計軸の選択（月別 / 顧客別 / 案件別）
  - 集計軸の変更は searchParams を更新する方式で実装する（Server Component 対応）
- [x] `getRevenueDetails` usecase を呼び出し、結果を `DataTable` で表示する
  - 月別: 列 = 期間, 金額, 件数
  - 顧客別: 列 = 顧客名, 金額, 件数
  - 案件別: 列 = 案件名, 金額, 件数
- [x] CSV エクスポートボタンを表示する — `/api/revenue/export?startDate=...&endDate=...&axis=...` へのリンク
- [x] `PageToolbar` で「売上明細」タイトルを表示する
...
- [x] `src/app/api/revenue/export/route.ts` を新規作成する
- [x] GET ハンドラを実装する:
...
- [x] `src/app/(dashboard)/revenue/forecast/page.tsx` を新規作成する
- [x] `auth()` でセッション取得
- [x] `getRevenueForecast` usecase を呼び出し、目標リストと実績データを取得する
- [x] 各目標に対して以下を表示する:
  ...
  - 進捗率（実績 / 目標、パーセント表示）
  - 着地予測（実績 + パイプライン合計）
- [x] `canPerform(role, "revenue", "setTarget")` が true のユーザーに目標設定フォームを表示する:
  - 期間の開始日・終了日入力
  - 目標金額入力（`MoneyInput` コンポーネント使用）
  - 送信ボタン（`setRevenueTargetAction` を呼び出す）
- [x] 既存目標の削除ボタンを認可ユーザーにのみ表示する（`deleteRevenueTargetAction` を呼び出す）
- [x] `PageToolbar` で「予実管理」タイトルを表示する
...
- [x] `src/app/(dashboard)/layout.tsx` のナビゲーション部分に「売上」リンクを追加する:
  - 「契約」リンクの後に配置する
...
- [x] `src/app/(dashboard)/labels.ts` に以下を追加する:
