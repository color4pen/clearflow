...
- [x] `src/infrastructure/schema.ts` に `actionItems` テーブルを追加:
...
- [x] `actionItemsRelations` を追加: organization, assignee, meeting, deal, inquiry, createdBy への one リレーション
- [x] `organizationsRelations` に `actionItems: many(actionItems)` を追加
- [x] `usersRelations` に `actionItemsAsAssignee: many(actionItems, { relationName: "actionItemsAsAssignee" })` と `actionItemsAsCreator: many(actionItems, { relationName: "actionItemsAsCreator" })` を追加
- [x] `meetingsRelations` に `actionItemsRef: many(actionItems)` を追加
- [x] `dealsRelations` に `actionItems: many(actionItems)` を追加
- [x] `inquiriesRelations` に `actionItems: many(actionItems)` を追加
...
- [x] `src/domain/models/actionItem.ts` を新設し以下の型を定義:
  ```
  ...
  ```
- [x] `src/domain/models/index.ts` に `actionItem.ts` の re-export を追加
- [x] ファイル内に Drizzle やインフラ層への import がないことを確認
...
- [x] `src/domain/authorization.ts` の `Entity` union 型に `"actionItem"` を追加
- [x] `PERMISSION_MATRIX` に `actionItem` エントリを追加:
  - `list`: ALL_ROLES
  - `view`: ALL_ROLES
...
- [x] `src/infrastructure/repositories/actionItemRepository.ts` を新設
- [x] `mapRow` ヘルパー関数を実装: DB 行 → `ActionItem` ドメインモデルへの変換
- [x] `create(data, tx?)` を実装: 必須フィールド + nullable FK を受け取り、INSERT → returning で `ActionItem` を返す
- [x] `findById(id, organizationId, tx?)` を実装: `id` AND `organizationId` で検索、見つからなければ null
- [x] `findByOrganization(organizationId, filters?)` を実装:
  - filters: `{ done?: boolean, assigneeId?: string, dealId?: string, meetingId?: string, inquiryId?: string }`
  - 各フィルタが指定されている場合のみ AND 条件に追加
  - `createdAt` の降順でソート
- [x] `update(id, organizationId, data, tx?)` を実装: Partial な更新データを受け取り、`updatedAt` を現在時刻に設定。返り値は `ActionItem | null`
- [x] `delete(id, organizationId, tx?)` を実装: `id` AND `organizationId` で削除。削除成功なら true、対象なしなら false
- [x] `findByDeal(dealId, organizationId)` を実装: 案件に紐づくアクションアイテム一覧を `createdAt` 降順で返す
- [x] `findByMeeting(meetingId, organizationId)` を実装: 商談に紐づくアクションアイテム一覧を `createdAt` 降順で返す
- [x] `src/infrastructure/repositories/index.ts` に `export * as actionItemRepository from "./actionItemRepository"` を追加
...
- [x] `src/application/usecases/createActionItem.ts` を新設:
...
- [x] `src/application/usecases/toggleActionItemDone.ts` を新設:
...
- [x] `src/application/usecases/updateActionItem.ts` を新設:
...
- [x] `src/application/usecases/deleteActionItem.ts` を新設:
  - 引数: `id`, `organizationId`, `actorId`
  - 存在確認 + org 一致チェック
...
- [x] `src/app/actions/actionItems.ts` を新設、`"use server"` ディレクティブ付き
- [x] `createActionItemAction` を実装:
  - `auth()` で認証チェック
...
- [x] `bunx drizzle-kit generate` を実行し、action_items テーブルの DDL マイグレーションファイルを生成する（`drizzle/0007_*.sql` として生成される）
- [x] 生成された SQL に CREATE TABLE, FK 制約, インデックスが含まれていることを確認する
...
- [x] `drizzle/0008_migrate_action_items_data.sql` を手動作成（ファイル名は生成された DDL の次番号に合わせる）
- [x] マイグレーション SQL の内容:
  ```sql
  ...
  ```
- [x] `meetings.action_items` カラムを削除しないことを確認（SQL に DROP COLUMN が含まれていない）
...
- [x] `src/domain/` 配下のファイルで `@/infrastructure` への import がないことを確認
- [x] authorization.ts の Entity 型に `"actionItem"` が含まれていることを確認
- [x] リポジトリ・ユースケース・サーバーアクションの各バレルファイルに新規ファイルの export が含まれていることを確認
