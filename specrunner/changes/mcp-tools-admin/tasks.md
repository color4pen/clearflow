...
- [ ] `src/domain/services/webhookUrlValidator.ts` を作成する
...
- [ ] `src/app/actions/webhooks.ts` から `PRIVATE_IP_PATTERNS`, `isPrivateHost`, `validateWebhookUrl` を削除し、`src/domain/services/webhookUrlValidator.ts` からの import に置き換える
...
- [ ] `src/app/api/mcp/tools/organization.ts` を作成する
...
- [ ] エラーハンドリング: 外側の try-catch で `handleToolError(error)` を返す
- [ ] import: `organizationRepository` は `@/infrastructure/repositories` のバレルから、`updateOrganization` は `@/application/usecases/updateOrganization` から個別 import する
...
- [ ] `src/app/api/mcp/tools/users.ts` を作成する
  - `getAuthInfo` ヘルパー
  - Zod スキーマ（5 操作分）:
...
- [ ] 全操作: `result.ok === false` → `toToolError(result.reason)`, 外側 try-catch → `handleToolError(error)`
...
- [ ] `src/app/api/mcp/tools/webhooks.ts` を作成する
  - `getAuthInfo` ヘルパー
  - Zod スキーマ（6 操作分）:
    - `listSchema`: operation: "list"
...
- [ ] 全操作共通: `canPerform(role, "organization", "manageWebhooks")` で認可チェック（admin のみ）
- [ ] list 操作:
...
- [ ] `src/app/api/mcp/tools/auditLogs.ts` を作成する
  - `getAuthInfo` ヘルパー
  - Zod スキーマ:
...
- [ ] `createMcpServer` 関数内に以下の呼び出しを追加:
...
- [ ] 新規ツールの register 関数を import に追加:
...
- [ ] `createMcpServer` 相当のセットアップに 4 つの register 関数呼び出しを追加する
- [ ] テスト内の期待値を更新:
...
- [ ] テスト記述（TC-025 のコメント）を「15 + 4 = 19 ツール」に更新する
...
- [ ] `bun test` green（既存テスト無変更で green）
- [ ] 新規テスト全件 green
