...
- [x] `src/app/(dashboard)/inquiries/InquiryStatusBadge.tsx` を作成する
- [x] props: `status: InquiryStatus` を受け取る
- [x] `statusLabels` を使用してラベルを表示する
- [x] スタイル: `inline-block`, `padding: 1px 7px`, `border-radius: 3px`, 背景 `#f5f5f5`, ボーダー `1px solid #e0e0e0`
- [x] Tailwind CSS クラスで実装する（`inline-block px-[7px] py-[1px] rounded-[3px] bg-[#f5f5f5] border border-[#e0e0e0] text-xs`）
...
- [x] `src/app/(dashboard)/inquiries/InquiryListView.tsx` を `"use client"` で作成する
- [x] Props 型を定義する:
  ```ts
  type InquiryRow = {
    id: string;
...
- [x] **フィルタロジック**: `useMemo` で 3 つのフィルタを AND 適用する
...
- [x] **テーブル描画**: CSS Grid で直接実装する（`DataTable` は使用しない）
...
- [x] **フッター**: `{filtered.length} 件` を表示
- [x] **空状態**: フィルタ結果が 0 件の場合「該当する引合はありません」を表示
...
- [x] `src/app/(dashboard)/inquiries/page.tsx` を修正する
- [x] `dealRepository` を import し、`dealRepository.findAllByOrganization(organizationId)` で全 deals を取得する（`Promise.all` で `listInquiries` と並列実行）
- [x] `inquiryId` → `dealId` の `Map` を構築する: `deals.filter(d => d.inquiryId).reduce(...)` で `Map<string, string>` を作成
- [x] `allInquiries` を `InquiryRow[]` 型にマッピングする: 各 inquiry に `dealId: dealMap.get(inquiry.id) ?? null` を追加
- [x] `sourceLabels` を `{ value, label }[]` 配列に変換して props として渡す
- [x] `PageToolbar` の title を維持し、フィルタ表示部分は削除する（`InquiryListView` 内に移動するため）
- [x] `InquiryListView` を描画し、`inquiries` と `sources` を props で渡す
- [x] 既存のフィルタロジック（`searchParams.status`、`filterLinkClass`、各 count）を削除する
- [x] `Date` オブジェクトはシリアライゼーション境界を超えられないため、`createdAt` を `string`（`toISOString()`）に変換して渡し、Client Component 側で表示フォーマットする
...
- [x] `src/app/(dashboard)/inquiries/[id]/InquiryStatusBanner.tsx` を作成する（Server Component）
- [x] Props 型:
  ```ts
  type Props = {
    status: InquiryStatus;
...
- [x] `src/app/(dashboard)/inquiries/[id]/InquiryInfoDisplay.tsx` を `"use client"` で作成する
- [x] Props 型:
  ```ts
  type Props = {
    inquiry: {
  ...
  ```
- [x] **表示モード**（デフォルト）: ラベル + 値のグリッドレイアウト（`grid-template-columns: 90px 1fr`）
  - 件名: テキスト表示
  - 経路: `sourceLabels` 変換表示
  - 内容: テキスト表示（Markdown の場合はそのまま plain text）
  - 作成日は `page.tsx` 側で表示する（InquiryInfoDisplay のスコープ外）
- [x] **編集ボタン**: `editable === true` の場合にヘッダー右に「編集」ボタンを表示
- [x] **編集モード**: ボタンクリックで `useState` を切り替え、既存の `InquiryInfoSection`（フォーム）を表示する
  - `InquiryInfoSection` は既存コンポーネントをそのまま利用する。ただし顧客セクションは独立化するため、`InquiryInfoSection` からは顧客関連 UI を削除する
- [x] 表示モードに戻る手段を提供する（「キャンセル」ボタン or フォーム保存後に自動切替）
...
- [x] `src/app/(dashboard)/inquiries/[id]/InquiryCustomerSection.tsx` を `"use client"` で作成する
- [x] Props 型:
  ```ts
  type Props = {
    inquiryId: string;
  ...
  ```
- [x] **顧客設定済み**: 顧客名を `/clients/[clientLinkId]` へのリンクとして表示
- [x] **顧客未設定 + 編集可能**: 「顧客が設定されていません」のエラーメッセージ + 顧客選択 `<select>` + 保存ボタン
  - 顧客選択は既存の `InquiryInfoSection` 内のロジック（既存顧客選択 / 新規作成）を移植する
  - 保存は `updateInquiryAction` を使用する
- [x] **顧客未設定 + 編集不可**: 「顧客が設定されていません」のメッセージのみ
- [x] セクションヘッダー: 「顧客」
...
- [x] `src/app/(dashboard)/inquiries/[id]/InquiryInfoSection.tsx` を修正する
- [x] 顧客セクション（`<dt>顧客</dt>` の `<div>` ブロック全体）を削除する
- [x] `clientMode` state と関連ロジックを削除する
- [x] Props 型から `clients`, `clientName`, `clientLinkId` を削除する（不要になるため）
- [x] **注意**: `InquiryInfoDisplay` の編集モードから呼び出されるため、フォーム機能自体は維持する
...
- [x] `src/app/(dashboard)/inquiries/[id]/InquiryMeetingSection.tsx` を作成する（Server-friendly, props 受け取り）
- [x] Props 型:
  ```ts
  type MeetingRow = {
    id: string;
...
- [x] **データ取得追加**: 既存の `Promise.all` に以下を追加する
...
- [x] **ステータスバナー**: パンくず下に `InquiryStatusBanner` を配置する
  - `hasPendingApproval`: `pendingRequest !== null` で判定
- [x] **2 カラムグリッド**: `<div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: "24px" }}>` で左右を配置
  - 左カラム:
...
- [x] 既存の縦積み `SectionCard` レイアウトを削除する
- [x] `InquiryInfoSection` の呼び出しを `InquiryInfoDisplay` に置き換える
- [x] Props の調整: `InquiryInfoDisplay` と `InquiryCustomerSection` に既存 props を分配する
...
- [x] `InquiryInfoDisplay.tsx` 内で `InquiryInfoSection` を import し、編集モード時に描画する
- [x] `InquiryInfoSection` に渡す props から `clients`, `clientName`, `clientLinkId` を削除する
- [x] 保存成功時に `InquiryInfoDisplay` の表示モードに戻すコールバックを実装する
...
- [x] `bun run build` を実行し、型エラーを修正する
- [x] 既存テスト（`bun test`）を実行し、失敗するテストを修正する
- [x] `bun run lint` を実行し、lint エラーを修正する
- [x] 特に以下の点を確認する:
