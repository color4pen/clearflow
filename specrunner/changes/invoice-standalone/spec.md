...
「入金確認」ボタン押下時に入金日入力ダイアログを表示しなければならない（MUST）。デフォルト値は現在日付（`YYYY-MM-DD`）。確認ボタン押下で `paidAt` を含めてステータス更新を実行する。入金日は本日以前の日付でなければならない（未来日付は拒否する）。
...
#### Scenario: 未来日付の入金日は拒否される

**Given** 請求のステータスが invoiced である
**When** 「入金確認」ボタンを押し、入金日に明日以降の日付を入力して確認ボタンを押す
**Then** バリデーションエラー「入金日は本日以前の日付を指定してください」が表示され、ステータスは更新されない
...
**Then** 請求が作成され、契約詳細ページ（`/contracts/{contractId}`）にリダイレクトされる
