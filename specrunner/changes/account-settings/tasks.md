...
- [x] `src/domain/models/auditLog.ts` の `AuditAction` union に `"user.updatePassword"` を追加する（`"user.updateRole"` の次に配置）
...
- [x] `src/infrastructure/repositories/userRepository.ts` に `findByIdForAuth(id: string, organizationId: string): Promise<UserWithPassword | null>` を追加する
- [x] WHERE は `and(eq(users.id, id), eq(users.organizationId, organizationId))` でテナント分離する
- [x] 返却型は既存の `UserWithPassword`（`User & { hashedPassword: string }`）を使用する
- [x] `select()` で全カラムを取得し、`findByEmailForAuth` と同じ projection で `UserWithPassword` を構築する
- [x] `.limit(1)` を付与する
...
- [x] `src/infrastructure/repositories/userRepository.ts` に `updateProfile(id: string, organizationId: string, data: { name: string }, tx?: Transaction): Promise<User | null>` を追加する
- [x] WHERE は `and(eq(users.id, id), eq(users.organizationId, organizationId))` で絞る
- [x] `.set({ name: data.name })` で name のみ更新する
- [x] `.returning()` は `updateRole` と同じ安全 projection（hashedPassword を含まない）を使用する
- [x] `tx ?? db` パターンで queryRunner を決定する（既存の `updateRole` と同じ）
...
- [x] `src/infrastructure/repositories/userRepository.ts` に `updatePassword(id: string, organizationId: string, hashedPassword: string, tx?: Transaction): Promise<boolean>` を追加する
- [x] WHERE は `and(eq(users.id, id), eq(users.organizationId, organizationId))` で絞る
- [x] `.set({ hashedPassword })` で hashedPassword のみ更新する
- [x] `.returning({ id: users.id })` で更新成功を確認し、`result.length > 0` を返す
- [x] `tx ?? db` パターンで queryRunner を決定する
...
- [x] `src/application/usecases/updateOwnProfile.ts` を新規作成する
- [x] 入力型: `{ userId: string; organizationId: string; name: string }`
- [x] 返却型: `UpdateOwnProfileResult = { ok: true } | { ok: false; reason: string }`
- [x] `userRepository.updateProfile(userId, organizationId, { name })` を呼び出す
- [x] 更新対象が見つからない場合（`null` 返却）は `{ ok: false, reason: "ユーザーが見つかりません" }` を返す
- [x] 監査ログは記録しない（D4: 表示名変更は監査対象外）
- [x] トランザクションは不要（単一操作）
- [x] `src/application/usecases/index.ts` に `export { updateOwnProfile } from "./updateOwnProfile"` を追加する
...
- [x] `src/application/usecases/changeOwnPassword.ts` を新規作成する
- [x] 入力型: `{ userId: string; organizationId: string; currentPassword: string; newPassword: string }`
- [x] 返却型: `ChangeOwnPasswordResult = { ok: true } | { ok: false; reason: string }`
- [x] `userRepository.findByIdForAuth(userId, organizationId)` で現在のハッシュを取得する
- [x] ユーザーが見つからない場合は `{ ok: false, reason: "ユーザーが見つかりません" }` を返す
- [x] `bcrypt.compare(currentPassword, user.hashedPassword)` で照合する。不一致なら `{ ok: false, reason: "現在のパスワードが正しくありません" }` を返す
- [x] `bcrypt.hash(newPassword, 12)` で新パスワードをハッシュする
- [x] `db.transaction` 内で以下を実行する:
...
- [x] `src/application/usecases/index.ts` に `export { changeOwnPassword } from "./changeOwnPassword"` を追加する
...
- [x] `src/app/actions/account.ts` を新規作成する
- [x] ファイル先頭に `"use server"` ディレクティブを記述する
- [x] `updateOwnProfileAction` を実装する:
...
- [x] `changeOwnPasswordAction` を実装する:
  - `auth()` で認証チェック
  ...
  - 返却型: `{ success: true } | { success: false; message: string }`
- [x] `canPerform` によるロールゲートは使用しない（全ロール許可）
- [x] `FormData` ベースのインターフェースとする（既存の Action パターンに合わせる）
...
- [x] `src/app/(dashboard)/account/page.tsx` を新規作成する:
...
- [x] `src/app/(dashboard)/account/ProfileForm.tsx` を新規作成する:
  - Client Component（`"use client"`）
  - name 入力フィールド（`FormField` + `Input` コンポーネント使用）
  - `updateOwnProfileAction` を `useActionState` で呼び出す
  - `SubmitButton` で送信
  - 成功・エラーメッセージの表示
- [x] `src/app/(dashboard)/account/PasswordForm.tsx` を新規作成する:
  - Client Component（`"use client"`）
...
- [x] `src/app/(dashboard)/SidebarNav.tsx` の `navItems` 配列に `{ href: "/account", label: "アカウント" }` を追加する
- [x] `adminOnly` は付けない（全ロール表示）
- [x] 配置位置は「設定」の前（管理系の前に本人向けを配置）
...
- [x] `src/__tests__/usecases/accountSettings.test.ts` を新規作成する（静的コード解析テスト、既存パターンに準拠）:
...
- [x] `src/__tests__/actions/accountActions.test.ts` を新規作成する（静的コード解析テスト）:
...
- [x] `src/__tests__/infrastructure/accountRepository.test.ts` を新規作成する（静的コード解析テスト）:
...
- [x] `src/__tests__/domain/accountAuditAction.test.ts` を新規作成する（静的コード解析テスト）:
