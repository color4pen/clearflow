...
admin ロールの操作者 SHALL 自組織内の無効化済みユーザーを再有効化できる。再有効化により `users.deactivated_at` が null に設定される。すでに有効なユーザーを再有効化しようとした場合はエラーを返し、DB 更新も監査ログ記録も行わない。
...
#### Scenario: admin がすでに有効なユーザーを再有効化しようとする

**Given** 操作者が admin ロール、対象ユーザーの `deactivated_at` が null（すでに有効）
**When** `reactivateUser` usecase を実行する
**Then** 「ユーザーはすでに有効です」エラーが返り、DB 更新も監査ログ記録も行われない
