...
`redirect_uri` が構文上不正または登録済み URI と一致しない場合、認可サーバーはその URI へリダイレクトしてはならない（MUST NOT）。この場合はエラーをエンドポイント上で直接（400 エラーページ等で）返さなければならない（SHALL）。RFC 9700 §4.1.2.1 に準拠する。

同意画面で使用する `organizationId` は Auth.js セッションから取得する（SHALL）。ユーザーが複数組織に所属する場合でも、認可リクエスト受信時のアクティブセッションに紐づく `organizationId` を使用する。これにより、発行されるトークンのテナントスコープが一意に決定される。
...
#### Scenario: 構文上有効だが未登録の redirect_uri は 400 エラーページを返す

**Given** `client_id` が登録済みである
**When** `GET /api/oauth/authorize` に構文上は有効だがそのクライアントの登録済み `redirect_uris` に含まれない URI を `redirect_uri` として指定してリクエストを送信する
**Then** 認可サーバーはその `redirect_uri` へリダイレクトせず、400 エラーページを直接返す（オープンリダイレクト防止）
...
#### Scenario: 期限切れ認可コードを拒否する

**Given** 認可コードが発行されてから 10 分以上経過している（D10 の有効期限超過）
**When** `POST /api/oauth/token` に `grant_type=authorization_code` と期限切れの認可コードを送信する
**Then** ステータス 400 が返り、`error=invalid_grant` が含まれる
...
### Requirement: ブラウザ発 OAuth エンドポイントに CORS ヘッダを設定する

`POST /api/oauth/register` および `POST /api/oauth/token` はブラウザから cross-origin リクエストを受信する。これらのエンドポイントは適切な CORS ヘッダ（`Access-Control-Allow-Origin`、`Access-Control-Allow-Methods`、`Access-Control-Allow-Headers`）を返さなければならない（SHALL）。claude.ai 等の外部 MCP クライアントのオリジンを許可する必要がある。CORS 設定が欠如するとブラウザのプリフライトチェックが失敗し、クライアントから接続できない。

#### Scenario: /api/oauth/register への CORS プリフライトが成功する

**Given** clearflow が起動している
**When** ブラウザが `OPTIONS /api/oauth/register` にプリフライトリクエストを送信する
**Then** ステータス 204 または 200 が返り、`Access-Control-Allow-Origin`、`Access-Control-Allow-Methods`（`POST` を含む）、`Access-Control-Allow-Headers` が含まれる

#### Scenario: /api/oauth/token への CORS プリフライトが成功する

**Given** clearflow が起動している
**When** ブラウザが `OPTIONS /api/oauth/token` にプリフライトリクエストを送信する
**Then** ステータス 204 または 200 が返り、`Access-Control-Allow-Origin`、`Access-Control-Allow-Methods`（`POST` を含む）、`Access-Control-Allow-Headers` が含まれる
