...
#### Scenario: decline 権限を持たないロールによる引合の却下

**Given** role が `member` のユーザーのトークン
**When** `inquiries` ツールを `operation: "update_status"` / `newStatus: "declined"` で呼び出す
**Then** isError: true の結果が返り、引合のステータスは変更されない（`canPerform(role, "inquiry", "decline")` により拒否）

#### Scenario: decline 権限を持つロールによる引合の却下は成功する

**Given** role が `admin` のユーザーのトークン、かつ対象の引合が存在する
**When** `inquiries` ツールを `operation: "update_status"` / `newStatus: "declined"` で呼び出す
**Then** 成功結果が返り、引合のステータスが `declined` に変更される
...
#### Scenario: MCP 経由の顧客更新が監査ログに記録される

**Given** 有効なトークンを持つ admin ユーザー
**When** `clients` ツールを `operation: "update"` で呼び出して顧客情報を更新する
**Then** `client.update` アクションの監査ログが記録される（usecase の `updateClient` が記録する）

#### Scenario: MCP 経由の顧客担当者更新が監査ログに記録される

**Given** 有効なトークンを持つ admin ユーザー
**When** `clients` ツールを `operation: "update_contact"` で呼び出して担当者情報を更新する
**Then** `client_contact.update` アクションの監査ログが記録される（usecase の `updateClientContact` が記録する）
