...
1. **MCP エンドポイント `/api/mcp`**: MCP Streamable HTTP transport（stateless、セッション ID なし）。`initialize` / `tools/list` / `tools/call` / `ping` に応答する。実装方式（公式 TypeScript SDK の採用か、プロトコル部分の最小実装か）は Next.js 16 route handler の制約（`node_modules/next/dist/docs/` を確認）を踏まえて design で決定し、判断根拠を記録する。**公式 SDK（`@modelcontextprotocol/sdk`）を採用する場合は、`package.json` の `dependencies` に明示的に追加する**（現状は開発ツールチェーンの推移的依存として node_modules に存在するのみで、本番ビルドでの可用性は保証されない）。最小実装を選ぶ場合は不要。
...
3. **パリティ規約（アダプタの核）**: 各ツールは対応する操作と**同じユースケース・同じ認可判定（canPerform）・同等の入力検証（Zod スキーマを可能な限り共有）・同じ監査記録**を通す。Server Action と MCP で挙動差・権限差を作らない。ツール専用のビジネスロジックを書かない。
   - **書き込みツール**: 対応する Server Action と同じユースケースを共有する。
   - **読み取りツール（対応する Server Action を持たない場合。例: `deals: get`）**: パリティ基準は「対応するユースケース（例: `getDeal`）を通し、canPerform で認可し、テナント分離を守る」と解釈する。UI が読み取りでユースケースを直接呼ぶのと同じ経路。
4. **ツール設計**: リソース単位のツール + operation 引数で集約し、ツール総数を抑える（全 85 操作を 1 操作 1 ツールにすると MCP クライアントのコンテキストを圧迫する）。本 request の範囲:
   ...
   - **顧客更新のユースケース新設（パリティ前提の整備）**: 現状 `updateClientAction` / `updateClientContactAction` はユースケース層を経ず `clientRepository.update()` / `updateContact()` を直接呼び、監査記録が無い。本 request で `updateClient` / `updateClientContact` ユースケース（監査記録 `client.update` / `client_contact.update` 付き。[[ent-audit-log]]）を新設し、**MCP ツールと既存 Server Action の両方をこのユースケース経由に揃える**。これにより顧客更新の監査漏れ（既存の潜在課題）も閉じる。既存の他 Server Action・テストは無変更で green を維持する。
...
- [ ] 顧客更新（updateClient / updateClientContact）が新設ユースケース経由で監査記録されることをテストで固定する（MCP ツール・既存 Server Action の両経路）
