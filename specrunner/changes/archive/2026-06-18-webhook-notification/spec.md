...
### Requirement: Webhook エンドポイントの URL は安全であること

登録される URL は https スキームのみ SHALL 許可する。私有 IP（RFC 1918）、ループバック、リンクローカルアドレスへの登録を拒否 MUST する。

#### Scenario: https URL のみ許可する

**Given** admin ユーザーが Webhook エンドポイントを登録しようとする
**When** URL が `http://example.com/webhook` である
**Then** 登録は拒否され「内部ネットワークの URL は登録できません」等のエラーメッセージが返される

#### Scenario: 私有 IP アドレスを拒否する

**Given** admin ユーザーが Webhook エンドポイントを登録しようとする
**When** URL が `https://192.168.1.1/webhook` である
**Then** 登録は拒否される

#### Scenario: リンクローカルアドレスを拒否する

**Given** admin ユーザーが Webhook エンドポイントを登録しようとする
**When** URL が `https://169.254.169.254/latest/meta-data/` である
**Then** 登録は拒否される
...
#### Scenario: step イベントのペイロードにステップ情報が含まれる

**Given** `step.approved` イベントが発生する
**When** ペイロードが構築される
**Then** `data.metadata` に `{ stepId: string, stepOrder: number, approverRole: string }` が含まれる
