...
4. **商談詳細 — MeetingInfoSection の分割**: 基本情報（日時、種別、場所）はヘッダー部分に読み取り表示する。編集は「編集」ボタンで MeetingInfoSection のフォームモードに切り替える（useState で表示/編集切替）。出席者とヒアリング情報は独立コンポーネントに分離する。場所（location）フィールドは基本情報のヘッダー内に含める
5. **顧客一覧 — カラム削減**: 7 カラムから 4 カラム（企業名, 業種, 関連案件数, 登録日）に変更する。DataTable の columns 定義で制御する。規模、担当者数、引き合い数カラムを削除する。不要になったデータ取得（担当者数、引き合い数のクエリ）がある場合は削除する
