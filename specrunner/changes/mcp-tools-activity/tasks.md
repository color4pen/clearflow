...
- [ ] `registerInteractionsTools(server: McpServer)` 関数をエクスポートする
- [ ] `getAuthInfo` ヘルパーを定義する（deals.ts / clients.ts と同一パターン）
- [ ] `z.discriminatedUnion("operation", [...])` で 4 operation のスキーマを定義する
- [ ] **create_meeting** operation:
...
- [ ] **record_contract_adjustment** operation:
...
- [ ] **record_invoice_adjustment** operation:
...
- [ ] エラー変換で内部詳細を漏らさない: usecase の reason をそのまま返す（usecase 側でメッセージは制御されている）。予期しない例外は handleToolError で固定文言に
...
- [ ] `registerTasksTools(server: McpServer)` 関数をエクスポートする
- [ ] `getAuthInfo` ヘルパーを定義する
- [ ] `z.discriminatedUnion("operation", [...])` で 7 operation のスキーマを定義する
- [ ] **list** operation:
...
- [ ] `registerWatchesTools(server: McpServer)` 関数をエクスポートする
- [ ] `getAuthInfo` ヘルパーを定義する
- [ ] `z.discriminatedUnion("operation", [...])` で 2 operation のスキーマを定義する
- [ ] **watch** operation:
...
- [ ] `registerNotificationsTools(server: McpServer)` 関数をエクスポートする
- [ ] `getAuthInfo` ヘルパーを定義する
- [ ] `z.discriminatedUnion("operation", [...])` で 2 operation のスキーマを定義する
- [ ] **list** operation:
...
- [ ] `import { registerInteractionsTools } from "./tools/interactions"` を追加
- [ ] `import { registerTasksTools } from "./tools/tasks"` を追加
- [ ] `import { registerWatchesTools } from "./tools/watches"` を追加
- [ ] `import { registerNotificationsTools } from "./tools/notifications"` を追加
...
- [ ] `createMeeting` usecase を `mock.module` で個別ファイルモックする
- [ ] `@/infrastructure/rateLimit` をモックする
- [ ] interactions ツールの `create_meeting` operation を実際に実行する
- [ ] `createMeeting` が呼ばれ、引数に `organizationId`, `actorId`, `kind: "meeting"`, `dealId` が含まれることを assert する
- [ ] `createMeeting` のモックが `{ ok: true, meeting: ... }` を返すとき、recordAudit が `interaction.create` アクションで呼ばれることを usecase 実装から推論（usecase 内で recordAudit されるため、usecase が呼ばれること = 監査記録されること）
- [ ] `afterAll` でモックを復元する
...
- [ ] `createMeeting` のモックが `{ ok: false, reason: "案件または引合のいずれかの指定が必要です" }` を返すよう設定する
- [ ] interactions ツールの `create_meeting` で dealId も inquiryId も指定せずに呼び出す
- [ ] ツール結果が `isError: true` であることを assert する
- [ ] `createMeeting` が呼ばれ、引数の dealId / inquiryId が両方 null/undefined であることを assert する
...
- [ ] `deleteActionItem` usecase を `mock.module` で個別ファイルモックする
- [ ] `@/infrastructure/rateLimit` をモックする
- [ ] **member ロールで delete を呼び、isError で拒否され usecase に到達しないことを assert する**（canPerform(member, actionItem, delete) = false）
- [ ] **admin ロールで delete を呼び、usecase に到達することを assert する**
- [ ] `createActionItem` usecase をモックし、**member ロールで create を呼び、usecase に到達することを assert する**（canPerform(member, actionItem, create) = true）
- [ ] `updateActionItemStatus` usecase をモックし、status 遷移が usecase に到達することを assert する
- [ ] `afterAll` でモックを復元する
...
- [ ] `watchDeal` usecase を `mock.module` で個別ファイルモックする
- [ ] 初回呼び出しで `{ ok: true, watch: ... }` を返す
- [ ] 2 回目の呼び出しで `{ ok: false, reason: "..." }` を返すよう設定する（一意性制約違反をシミュレート）
- [ ] ツール結果が `isError: true` であることを assert する
- [ ] `afterAll` でモックを復元する
...
- [ ] `getNotifications` usecase を `mock.module` で個別ファイルモックする
- [ ] `userRepository` から `findById` をモックし、notificationsLastSeenAt を含むユーザーレコードを返す
- [ ] notifications ツールの `list` operation を authInfo.userId = "user-A" で実行する
- [ ] `getNotifications` が呼ばれた際の引数 `userId` が "user-A" であることを assert する
- [ ] `getNotifications` が呼ばれた際の引数 `organizationId` が authInfo の organizationId と一致することを assert する
- [ ] `afterAll` でモックを復元する
...
- [ ] **監査記録の実行検証**: tasks ツールの `create` operation を実行し、`createActionItem` usecase が `organizationId` と `actorId` を受け取って呼ばれることを assert する（usecase 内で recordAudit が呼ばれる = 監査記録される）
- [ ] **テナント分離の実行検証**: 2 つの異なる organizationId を持つ authInfo で同一操作を呼び、usecase に渡される organizationId がそれぞれ正しいことを assert する
- [ ] interactions の `create_meeting` でも同様に organizationId 伝播を検証する
- [ ] watches の `watch` でも organizationId 伝播を検証する
- [ ] `afterAll` でモックを復元する
