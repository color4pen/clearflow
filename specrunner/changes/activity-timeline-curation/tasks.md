...
- [ ] `src/domain/models/auditLog.ts` の `AuditMetadataMap` に以下 3 エントリを追加する:
...
- [ ] `bun run typecheck` を実行し、既存の `recordAudit` 呼び出し（`updateDealPhase.ts` / `updateContractStatus.ts`）が型エラーにならないことを確認する（既に正しい metadata を渡している）
...
- [ ] `src/application/usecases/updateInvoiceStatus.ts` の `recordAudit` 呼び出しに `metadata: { fromStatus: invoice.status, toStatus: data.newStatus }` を追加する
- [ ] `bun run typecheck` を実行し型エラーがないことを確認する
...
- [ ] `src/lib/activityConfig.ts` にタイムライン表示対象アクションのホワイトリスト定数を追加する:
  ```
...
- [ ] `src/lib/activityAggregator.ts` を新規作成し、以下を実装する:
  - `TimelineEntry` 型の定義:
    ```typescript
    type TimelineEntry = {
      id: string;
...
- [ ] `aggregateTimeline` は純粋関数とし、外部依存を持たない
...
- [ ] `src/application/usecases/getDealActivity.ts` を改修する:
...
- [ ] `DealActivityResult` と `TimelineEntry` を適切に export する
...
- [ ] `src/lib/activityLabels.ts` を改修する:
...
- [ ] 集約件数の表示: `count > 1` の場合に「(N件)」を付加する処理は UI 側（`DealActivitySection`）で行う
...
- [ ] `src/app/(dashboard)/deals/[id]/DealActivitySection.tsx` を改修する:
...
- [ ] `src/app/(dashboard)/deals/[id]/page.tsx` の `DealActivitySection` 呼び出し箇所で、型の不整合がないことを確認する（`getDealActivity` が `TimelineEntry[]` を返すようになるため自然に整合する）
...
- [ ] `src/__tests__/usecases/dealActivity.test.ts` を改修する:
...
- [ ] `src/__tests__/usecases/dealActivity.dynamic.test.ts` を新規作成する（`getNotifications.dynamic.test.ts` を参考に `mock.module` 方式で実装）
- [ ] 以下のテストケースを実装する:
...
- [ ] 集約ロジック単体のテストも追加する（`activityAggregator` の純粋関数テスト）
- [ ] `bun test` を実行し全テストが通ることを確認する
...
- [ ] 依存方向（actions/RSC → usecases → domain / infrastructure）が遵守されていることを確認する: `activityAggregator.ts`（lib 層）が domain/infrastructure を直接参照していないこと
