...
- 新規作成 6 箇所のブラケットリンクを BTN_PRIMARY の Link に置換し contracts に 1 箇所新設（計 7 箇所）
...
**決定**: 6 箇所のブラケットリンクを `<Link href="..." className={BTN_PRIMARY}>＋ テキスト</Link>` に置換する。`BTN_PRIMARY` は既存 `src/app/(dashboard)/styles.ts` の定数を使用する。contracts/page.tsx には `<Link href="/contracts/new" className={BTN_PRIMARY}>＋ 新規作成</Link>` を PageToolbar の `actions` として追加する。
...
**既知の制約**: `/contracts/new` ページは `?dealId=` クエリパラメータが必須であり、パラメータなしでアクセスすると `notFound()` を返す。今回追加するリンクはパラメータなしの URL (`/contracts/new`) を指す（仕様通り）。
...
- **[Risk] contracts の新規作成導線が 404 を返す**: `/contracts/new` は `?dealId=` 必須。今回追加するリンクはパラメータなし URL を指すため、アクセス先が `notFound()` を返す。
  **Mitigation**: 本仕様の記載通りに実装する（スコープ外につき遷移先の修正は行わない）。
