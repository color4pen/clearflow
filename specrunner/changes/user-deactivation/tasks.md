...
- [ ] `src/infrastructure/schema.ts` の `users` テーブル定義に `deactivatedAt: timestamp("deactivated_at")` を追加する（nullable、デフォルトなし）。`createdAt` の後に配置する
- [ ] `bun run db:generate` で差分マイグレーションを生成する
- [ ] 生成されたマイグレーション SQL が `ALTER TABLE ... ADD COLUMN "deactivated_at"` のみであることを確認する（他テーブル・他カラムの変更を含まないこと）
...
- [ ] `src/domain/models/user.ts` の `User` 型に `deactivatedAt: Date | null` を追加する
- [ ] `src/infrastructure/repositories/userRepository.ts` の全 select 句（`findByOrganization`, `updateRole`, `findById`, `create`, `updateProfile`）に `deactivatedAt: users.deactivatedAt` を追加する
- [ ] `findByEmailForAuth` の返り値にも `deactivatedAt` を追加する（UserWithPassword 型は User を extends しているため自動的に含まれるが、select 句でフィールドを明示する）
- [ ] `findByIdForAuth` の返り値にも同様に `deactivatedAt` を追加する
...
- [ ] `src/infrastructure/repositories/userRepository.ts` に `deactivate(id: string, organizationId: string, tx?: Transaction): Promise<User | null>` を追加する。`deactivated_at = new Date()` に設定し、`WHERE (id, organizationId)` で絞る。更新後の User を returning で返す
- [ ] 同ファイルに `reactivate(id: string, organizationId: string, tx?: Transaction): Promise<User | null>` を追加する。`deactivated_at = null` に設定し、`WHERE (id, organizationId)` で絞る。更新後の User を returning で返す
- [ ] returning の select 句は既存の `updateRole` と同じフィールド（`id, email, name, organizationId, role, notificationsLastSeenAt, createdAt` + `deactivatedAt`）を返す
...
- [ ] `src/domain/models/auditLog.ts` の `AuditAction` union 型に `"user.deactivate"` と `"user.reactivate"` を追加する。`"user.updatePassword"` の後に配置する
...
- [ ] `src/domain/authorization.ts` の `organization` エンティティの権限マトリクスに `deactivateUser: ADMIN_ONLY` を追加する。`createUser` の後に配置する
...
- [ ] `src/application/usecases/deactivateUser.ts` を作成する
- [ ] 入力型: `{ actorId: string; targetUserId: string; organizationId: string }`
- [ ] 戻り値型: `DeactivateUserResult = { ok: true } | { ok: false; reason: string }`
- [ ] ガード 1: `actorId === targetUserId` の場合は `{ ok: false, reason: "自分自身は無効化できません" }` を返す
- [ ] ガード 2: `findById(targetUserId, organizationId)` で対象ユーザーを取得。見つからなければ `{ ok: false, reason: "ユーザーが見つかりません" }`
- [ ] ガード 3: 対象ユーザーが admin の場合、`findByOrganization(organizationId)` で有効な admin（`deactivatedAt === null`）を数え、対象以外に admin がいなければ `{ ok: false, reason: "組織に最低1人の管理者が必要です" }`
- [ ] `db.transaction` 内で `userRepository.deactivate(targetUserId, organizationId, tx)` と `recordAudit({ action: "user.deactivate", targetType: "user", targetId: targetUserId, actorId, organizationId }, tx)` を実行する
- [ ] `src/application/usecases/index.ts` に `export { deactivateUser } from "./deactivateUser"` を追加する
...
- [ ] `src/app/actions/users.ts` に `deactivateUserAction` を追加する
...
- [ ] `src/app/actions/users.ts` に `reactivateUserAction` を追加する
...
- [ ] `import` に `deactivateUser`, `reactivateUser` を追加する
...
- [ ] `src/app/(dashboard)/settings/users/UserDeactivateButton.tsx` クライアントコンポーネントを作成する
  ...
  - エラー時はエラーメッセージを表示する
  - 送信中は disabled にする
- [ ] `src/app/(dashboard)/settings/users/page.tsx` を更新する
...
- [ ] `src/__tests__/usecases/userManagement.test.ts` に `deactivateUser` usecase の静的コード解析テストを追加する
...
- [ ] 同ファイルに `reactivateUser` usecase の静的コード解析テストを追加する
  - `db.transaction` の使用
  - `recordAudit` がトランザクション内で呼ばれる
