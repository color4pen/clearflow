...
- [x] `src/domain/models/invoice.ts` を新規作成し、以下を定義する:
...
- [x] `src/domain/models/index.ts` に `export type { InvoiceStatus, Invoice } from "./invoice"` を追記する
...
- [x] `src/infrastructure/schema.ts` に `invoiceStatusEnum` を追加する: `pgEnum("invoice_status", ["scheduled", "invoiced", "paid", "overdue"])`（`renewalTypeEnum` の後に配置）
- [x] `src/infrastructure/schema.ts` に `invoices` テーブルを追加する（`contracts` テーブルの後に配置）:
...
- [x] `src/infrastructure/schema.ts` に `invoicesRelations` を追加する: `organization` への `one()`、`contract` への `one()`
- [x] `contractsRelations` に `invoices: many(invoices)` を追加する（`one()` から `one, many` に変更）
- [x] `organizationsRelations` に `invoices: many(invoices)` を追加する
...
- [x] `src/infrastructure/repositories/invoiceRepository.ts` を新規作成する
- [x] `mapRow` 関数: DB 行を `Invoice` 型にマッピングする
- [x] `create(data, tx?)`: 請求を作成する。`organizationId` を含む
- [x] `findById(id, organizationId, tx?)`: ID と organizationId で請求を取得する
- [x] `findAllByContract(contractId, organizationId, tx?)`: 契約に紐づく請求一覧を取得する。`createdAt` 昇順
- [x] `update(id, organizationId, data, tx?)`: 請求を更新する。`updatedAt` を現在日時に更新
- [x] `updateStatus(id, organizationId, status, additionalFields?, tx?)`: ステータスを更新する。`additionalFields` で `invoicedAt` / `paidAt` を受け取る。`updatedAt` を現在日時に更新
- [x] `sumAmountByContract(contractId, organizationId, tx?)`: 契約に紐づく全請求の金額合計を SQL の `SUM(amount)` で返す。請求が存在しない場合は `0` を返す
- [x] `src/infrastructure/repositories/index.ts` に `export * as invoiceRepository from "./invoiceRepository"` を追記する
...
- [x] `src/domain/services/invoiceTransition.ts` を新規作成する
- [x] `VALID_INVOICE_TRANSITIONS` 定数を定義する:
  - `scheduled`: `["invoiced"]`
  - `invoiced`: `["paid", "overdue"]`
  - `paid`: `[]`（終端状態）
  - `overdue`: `[]`（終端状態）
- [x] `validateInvoiceTransition(from: InvoiceStatus, to: InvoiceStatus): { ok: true } | { ok: false; reason: string }` 関数をエクスポートする
- [x] `src/domain/services/index.ts` に `export { validateInvoiceTransition } from "./invoiceTransition"` を追記する（既存の `contractTransition`・`inquiryTransition`・`dealTransition` と同じパターン）
...
- [x] `src/application/usecases/createInvoice.ts` を新規作成する
- [x] 引数: `contractId`, `organizationId`, `actorId`, `title`, `amount`, `dueDate?`, `notes?`
- [x] 戻り値: `{ ok: true; invoice: Invoice } | { ok: false; reason: string }`
- [x] 処理フロー:
...
- [x] `src/application/usecases/index.ts` に `export { createInvoice } from "./createInvoice"` を追記する
...
- [x] `src/application/usecases/updateInvoiceStatus.ts` を新規作成する
- [x] 引数: `invoiceId`, `organizationId`, `actorId`, `newStatus`
- [x] 戻り値: `{ ok: true; invoice: Invoice } | { ok: false; reason: string }`
- [x] 処理フロー:
...
- [x] `src/application/usecases/index.ts` に `export { updateInvoiceStatus } from "./updateInvoiceStatus"` を追記する
...
- [x] `src/application/usecases/listInvoicesByContract.ts` を新規作成する
- [x] 引数: `contractId`, `organizationId`
- [x] 戻り値: `Invoice[]`
- [x] `invoiceRepository.findAllByContract` を呼び出して返す
- [x] `src/application/usecases/index.ts` に `export { listInvoicesByContract } from "./listInvoicesByContract"` を追記する
...
- [x] `src/app/(dashboard)/labels.ts` に `invoiceStatusLabels` を追加する:
...
- [x] `src/app/actions/invoices.ts` を新規作成する
- [x] `"use server"` 宣言を記述する
- [x] `createInvoiceAction(formData: FormData)`:
  - 認証チェック（`auth()`）
...
- [x] `updateInvoiceStatusAction(invoiceId: string, newStatus: string)`:
  - 認証チェック
  - 権限チェック（admin / manager のみ）
  ...
  - `revalidatePath` で契約詳細ページを再検証
- [x] `listInvoicesByContractAction(contractId: string)`:
  - 認証チェック
...
- [x] 請求一覧コンポーネントを作成する（`src/app/(dashboard)/contracts/[id]/InvoiceSection.tsx`）:
...
- [x] 「請求を追加」ボタンを配置する（admin / manager のみ表示。契約が active の場合のみ）
- [x] ステータス変更ボタンを各行に配置する:
  - `scheduled` の行: 「請求書発行」ボタン（→ invoiced）
  - `invoiced` の行: 「入金確認」ボタン（→ paid）と「期日超過」ボタン（→ overdue）
  - `paid` / `overdue` の行: ボタンなし（終端状態）
  - admin / manager のみ表示
- [x] `src/app/(dashboard)/contracts/[id]/page.tsx` にセクションを組み込む
...
- [x] 請求追加モーダルコンポーネントを作成する（`src/app/(dashboard)/contracts/[id]/CreateInvoiceModal.tsx`）:
...
- [x] `src/infrastructure/seed.ts` — truncation 順序に `invoices` を追加する（`contracts` の前に `await db.delete(invoices)` を追加）
- [x] `src/infrastructure/seed.ts` — `invoices` テーブルのインポートを追加する
- [x] DX推進プロジェクト契約に対して請求3件を追加する:
...
- [x] 請求の `organizationId` は契約と同じ組織IDを使用する
- [x] 契約の insert から返される ID を使用するか、returning を追加して contractId を取得する
...
- [x] `src/__tests__/static/projectStructure.test.ts` のモデルファイル一覧（TC-031）に `"domain/models/invoice.ts"` を追記する
- [x] テナント分離テストを追加する:
...
- [x] `bunx drizzle-kit generate` を実行してマイグレーションファイルを生成する（`invoice_status` enum の追加、`invoices` テーブルの追加）
