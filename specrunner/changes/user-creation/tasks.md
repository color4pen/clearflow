...
- [ ] `src/domain/models/auditLog.ts` の `AuditAction` 型に `"user.create"` を追加する（既存の `"user.updateRole"` の近くに配置）
...
- [ ] `src/domain/authorization.ts` の `PERMISSION_MATRIX.organization` に `createUser: ADMIN_ONLY` を追加する
...
- [ ] `src/infrastructure/repositories/userRepository.ts` に `create` 関数を追加する
- [ ] 引数: `data: { organizationId: string; email: string; name: string; role: Role; hashedPassword: string }` と `tx?: Transaction`
- [ ] `tx ?? db` パターンでトランザクション対応する（既存の `updateRole` と同じパターン）
- [ ] `.returning()` で `User` 型のフィールドを返す（`id`, `email`, `name`, `organizationId`, `role`, `notificationsLastSeenAt`, `createdAt`）
- [ ] 戻り値: `Promise<User>`
...
- [ ] `src/application/usecases/createUser.ts` を新規作成する
- [ ] 結果型: `CreateUserResult = { ok: true; user: User } | { ok: false; reason: string }`
- [ ] 引数: `{ organizationId: string; actorId: string; email: string; name: string; role: Role; password: string }`
- [ ] 処理フロー:
...
- [ ] `src/application/usecases/index.ts` に `export { createUser } from "./createUser"` を追加する
...
- [ ] `src/app/actions/users.ts` に `createUserAction` を追加する
- [ ] `createUser` usecase を import に追加する
- [ ] zod スキーマ `createUserSchema` を定義する:
  - `email`: `z.string().email("有効なメールアドレスを入力してください")`
  - `name`: `z.string().min(1, "名前は必須です")`
  - `role`: `z.enum(["admin", "member", "manager", "finance"])`
  - `password`: `z.string().min(8, "パスワードは8文字以上で入力してください")`
- [ ] 関数シグネチャ: `async function createUserAction(prevState: CreateUserState, formData: FormData): Promise<CreateUserState>`
- [ ] 処理フロー:
  1. `auth()` で認証チェック
...
- [ ] `src/app/(dashboard)/settings/users/CreateUserForm.tsx` を新規作成する
- [ ] `"use client"` ディレクティブを付ける
- [ ] `useActionState` を使って `createUserAction` を呼び出す（WebhookCreateForm と同じパターン）
- [ ] フォームフィールド:
...
- [ ] 共通コンポーネント（`FormField`, `Input`, `Select`, `SubmitButton`, `preventEnterSubmit`）を使用する
- [ ] エラー表示: `state?.success === false` の場合にエラーメッセージを表示する
- [ ] 成功表示: `state?.success === true` の場合に成功メッセージを表示し、フォームをリセットする
...
- [ ] `src/app/(dashboard)/settings/users/page.tsx` に `CreateUserForm` を import する
- [ ] ユーザー一覧の上部に `SectionCard` で囲んだ `CreateUserForm` を配置する
- [ ] ページの admin ガードは既存のまま（追加の権限チェック不要）
...
- [ ] `src/__tests__/usecases/userManagement.test.ts` に `createUser` usecase のテストを追加する（静的コード解析パターン）:
...
- [ ] `src/__tests__/settings/userSettingsActions.test.ts` に `createUserAction` のテストを追加する（静的コード解析パターン）:
...
- [ ] `src/__tests__/domain/authorization.test.ts` に `createUser` 認可のテストを追加する:
...
- [ ] `src/__tests__/static/auditRecorder.test.ts` で `"user.create"` が `AuditAction` に含まれることを確認する（既存テストのパターンに従って必要な場合のみ追加）
