...
- [ ] シグネチャ: `listInvoicesByOrganization(data: { organizationId: string; status?: InvoiceStatus; paidAtFrom?: Date; paidAtTo?: Date; issueDateFrom?: Date; issueDateTo?: Date }): Promise<Invoice[]>`
- [ ] 内部で `invoiceRepository.findAllByOrganization(data.organizationId, { status, paidAtFrom, paidAtTo, issueDateFrom, issueDateTo })` を呼ぶ
...
- [ ] `src/domain/models/dashboard.ts` を新規作成する
- [ ] 以下の型を定義する:
  ```typescript
  ...
  ```
- [ ] `src/domain/models/index.ts` に `export type { DashboardActionItem } from "./dashboard";` を追加する
- [ ] `src/application/usecases/getDashboardActions.ts` を新規作成する
- [ ] シグネチャ: `getDashboardActions(organizationId: string, userRole: string): Promise<DashboardActionItem[]>`
- [ ] 内部で以下を `Promise.all` で並列取得する:
...
- [ ] `src/domain/models/dashboard.ts` に以下の型を追加する:
  ```typescript
  ...
  ```
- [ ] `src/domain/models/index.ts` に `PipelineSummaryItem` のエクスポートを追加する
- [ ] `src/application/usecases/getPipelineSummary.ts` を新規作成する
- [ ] シグネチャ: `getPipelineSummary(organizationId: string): Promise<{ summary: PipelineSummaryItem[]; deals: DealWithDetails[] }>`
- [ ] 内部で `listDeals(organizationId)` を呼び、フェーズごとに案件数と `estimatedAmount` の合計を集計する
- [ ] `estimatedAmount` が null の案件は金額 0 として集計する
- [ ] 全 5 フェーズ（`proposal_prep`, `proposed`, `negotiation`, `won`, `lost`）を含む結果を返す（案件 0 件のフェーズも含む）
- [ ] `deals` も返す（停滞案件フィルタで再利用するため）
- [ ] `src/application/usecases/index.ts` にエクスポートを追加する
...
- [ ] Props: `{ actions: DashboardActionItem[]; pipelineSummary: PipelineSummaryItem[]; recentActivities: AuditLog[]; staleDeals: DealWithDetails[] | null }`
- [ ] アクション待ちリストセクション: `SectionCard` 内に `DataTable` またはリストで表示。種別（承認/TODO/引合）をアイコンまたはラベルで区別。各アイテムに対象エンティティへのリンクを含む
- [ ] パイプラインサマリセクション: `SectionCard` 内にフェーズ別カード or テーブルで表示。フェーズ名（`phaseLabels` を使用）、件数、金額合計。各フェーズは `/deals?phase=xxx` へのリンク
- [ ] 直近の活動セクション: `SectionCard` 内に監査ログを表示。`targetType` と `targetId` からエンティティ詳細へのリンクを生成する。リンク生成ルール — `targetType` が `deal` → `/deals/{targetId}`, `request` → `/requests/{targetId}`, `inquiry` → `/inquiries/{targetId}`, `contract` → `/contracts/{targetId}`, `invoice` → `null`（請求は契約詳細内のため直リンク不可）, その他 → `null`
- [ ] 停滞案件セクション（`staleDeals` が null でない場合のみ表示）: `SectionCard` 内に案件名、フェーズ、最終更新日を表示。各行は `/deals/{id}` へのリンク
- [ ] 既存の UI コンポーネント（`PageToolbar`, `SectionCard`, `DataTable`）を活用する
...
- [ ] `page.tsx` の経理ダッシュボード向けデータ取得を実装する（`Promise.all` で並列化）:
...
- [ ] 月初・翌月初・翌々月初の計算: `new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1))` 等で UTC ベースで算出する
- [ ] paid 請求の `amount` を合計して今月の売上サマリ金額を算出する
- [ ] `src/app/(dashboard)/dashboard/FinanceDashboard.tsx` をクライアントコンポーネント（`"use client"`）として新規作成する
- [ ] Props: `{ overdueInvoices: Invoice[]; unpaidInvoices: Invoice[]; monthlySalesTotal: number; upcomingInvoices: Invoice[] }`
- [ ] 期日超過の請求セクション: `SectionCard` 内に dueDate 昇順で表示。金額、タイトル、支払期日を含む
- [ ] 未入金の請求セクション: `SectionCard` 内に dueDate 昇順で表示
- [ ] 今月の売上サマリセクション: 合計金額を `¥xxx,xxx` 形式で大きく表示する
- [ ] 請求予定セクション: `SectionCard` 内に issueDate 順で表示
- [ ] 各請求行に契約詳細へのリンク（`/contracts/{contractId}`）を含む
...
- [ ] `src/app/(dashboard)/layout.tsx` のナビゲーション `<nav>` 内の先頭に以下を追加する:
  ```tsx
  <Link href="/dashboard" className="text-text-on-dark-secondary hover:text-white text-sm">
    ダッシュボード
  </Link>
  ```
- [ ] 既存のナビゲーションリンク（顧客、引き合い、案件、契約、売上、申請一覧）の順序・スタイルは変更しない
...
- [ ] `src/__tests__/usecases/dashboardActions.test.ts` を新規作成する
- [ ] テストケース: ロール一致の pending 承認リクエストが含まれること
- [ ] テストケース: ロール不一致の承認リクエストが除外されること
- [ ] テストケース: `done === false` のアクションアイテムのみ含まれること
- [ ] テストケース: `status === "new"` の引合のみ含まれること
- [ ] テストケース: 期日昇順でソートされ、null は末尾であること
- [ ] `src/__tests__/usecases/pipelineSummary.test.ts` を新規作成する
- [ ] テストケース: 全 5 フェーズの集計結果が返されること
- [ ] テストケース: estimatedAmount が null の案件は金額 0 扱いであること
- [ ] `src/__tests__/usecases/listInvoicesByOrganization.test.ts` を新規作成する
- [ ] テストケース: フィルタなしで全請求が返されること
- [ ] テストケース: status フィルタが適用されること
- [ ] テストケース: paidAt 期間フィルタが適用されること
- [ ] テストケース: issueDate 期間フィルタが適用されること
...
- [ ] `bun run test` が全件 green であることを確認する
