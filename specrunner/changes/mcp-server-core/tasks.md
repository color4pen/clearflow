...
- [ ] `design/static/modules.md` に `mod-mcp` セクションを追加する:
  ```
  ## MCP {#mod-mcp}
  責務: MCP プロトコルの受付・ツール登録・Bearer 認証解決・認可チェック・ユースケース委譲。ビジネスロジックを持たない。
  実装: src/app/api/mcp/
  ```
- [ ] `design/static/dependencies.md` のプレゼンテーション層セクションに以下の許可依存を追加する:
  - `[[mod-mcp]] -> [[mod-usecase]]`
  - `[[mod-mcp]] -> [[mod-auth]]`
  - `[[mod-mcp]] -> [[mod-authz]]`
  - `[[mod-mcp]] -> [[mod-model]]`
...
- [ ] `bunx aozu export rules --out design/rules.json` で rules.json を再生成する
- [ ] `bun test src/__tests__/static/architecture.test.ts` で architecture test が green になることを確認する
...
- [ ] `src/app/api/mcp/route.ts` を作成する。`POST` handler を export する
- [ ] route handler 内で `request.headers.get("Authorization")` を取得し、`resolveBearer` を呼び出す
- [ ] `resolveBearer` が null を返した場合、HTTP 401 レスポンスを返す（`new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401 })`）
- [ ] 認証成功時、`WebStandardStreamableHTTPServerTransport` を stateless モード（`sessionIdGenerator: undefined`）、`enableJsonResponse: true` で生成する
- [ ] モジュールレベルのシングルトン `McpServer` を作成し、`serverInfo` に `{ name: "clearflow", version: "1.0.0" }` を設定する
- [ ] transport を `McpServer` に connect し、`transport.handleRequest(request)` の結果を Response として返す
- [ ] GET handler も export し、stateless モードでの適切なレスポンス（SDK が 405 を返す）を処理する
- [ ] 認証で解決した `{ userId, organizationId, role }` を transport の `handleRequest` に `authInfo` として渡し、ツールハンドラ内で参照できるようにする。SDK の `HandleRequestOptions.authInfo` を使用する
...
- [ ] `src/app/api/mcp/tools/inquiries.ts` を作成する
- [ ] `registerInquiriesTools(server: McpServer)` 関数を export する
- [ ] `inquiries` ツールを `server.registerTool` で登録する。`inputSchema` に operation の discriminated union を定義する:
...
- [ ] `update_status` で `newStatus: "converted"` の場合:
  - `canPerform(role, "inquiry", "convert")` で認可チェック
  - usecase の結果に `pendingApproval` がある場合、ツール結果のテキストに承認待ちである旨を含める
- [ ] `update_status` で `newStatus: "declined"` の場合:
  - `canPerform(role, "inquiry", "decline")` で認可チェック
- [ ] `create` で `newClientName` が指定され `clientId` が未指定の場合、Server Action と同様に `createClient` usecase を先に呼び出して顧客を作成する
- [ ] route.ts の McpServer 初期化部分で `registerInquiriesTools(server)` を呼び出す
...
- [ ] `src/app/api/mcp/tools/deals.ts` を作成する
- [ ] `registerDealsTools(server: McpServer)` 関数を export する
- [ ] `deals` ツールを登録する。inputSchema:
...
- [ ] 各 operation のハンドラで canPerform チェック:
...
- [ ] `get` 操作の実装:
  ...
  - 結果を統合して返す
- [ ] `create` で `inquiryId` も `clientId` もない場合のエラーハンドリング（Server Action と同様）
- [ ] 書き込み操作に `checkRateLimit` を適用する
- [ ] route.ts で `registerDealsTools(server)` を呼び出す
...
- [ ] `src/app/api/mcp/tools/clients.ts` を作成する
- [ ] `registerClientsTools(server: McpServer)` 関数を export する
- [ ] `clients` ツールを登録する。inputSchema:
...
- [ ] `src/app/api/mcp/errors.ts` を作成する
- [ ] `toToolError(message: string): CallToolResult` ヘルパーを実装する。`{ isError: true, content: [{ type: "text", text: message }] }` を返す
- [ ] `toToolSuccess(data: unknown): CallToolResult` ヘルパーを実装する。`{ content: [{ type: "text", text: JSON.stringify(data, null, 2) }] }` を返す
- [ ] `handleToolError(error: unknown): CallToolResult` ヘルパーを実装する。予期しない例外を安全なエラーメッセージに変換する。スタックトレースを含めない
- [ ] 全ツールハンドラからこれらのヘルパーを使用する
...
- [ ] `src/__tests__/mcp/mcpProtocol.test.ts` を作成する
- [ ] テスト用に `McpServer` と `WebStandardStreamableHTTPServerTransport` を直接使用する（route handler を経由しない transport レベルのテスト）
- [ ] テストケース: `initialize` → `tools/list` → `tools/call` の一連の JSON-RPC 交換
...
- [ ] テストケース: 未知のツール名での tools/call がエラーを返す
- [ ] テストケース: 不正な JSON-RPC メッセージが適切なエラーを返す
...
- [ ] `src/__tests__/mcp/mcpAuth.test.ts` を作成する
- [ ] route handler（`src/app/api/mcp/route.ts`）の POST 関数を直接 import してテストする、または route handler と同等のロジックを抽出した関数をテストする
- [ ] テストケース: Authorization ヘッダなし → 401
- [ ] テストケース: `Authorization: Bearer invalid` → 401
- [ ] テストケース: 失効済みトークン → 401
- [ ] テストケース: 無効化されたユーザーのトークン → 401
- [ ] テストケース: 有効なトークン → 200（正常な JSON-RPC レスポンス）
- [ ] `resolveBearer` のモックを使用して、認証ロジックを分離してテストする
...
- [ ] `src/__tests__/mcp/mcpAuthorization.test.ts` を作成する
- [ ] 各ロール（admin / manager / member / finance）× 各操作の組み合わせで canPerform の結果が Server Action と一致することをテストする
- [ ] テストケース: member による inquiry.delete → isError
- [ ] テストケース: member による deal.create → isError
...
- [ ] `src/__tests__/mcp/mcpTenantIsolation.test.ts` を作成する
- [ ] テストケース: テナント A のトークンでテナント B の引合を update → エラーまたは not found
- [ ] テストケース: テナント A のトークンでテナント B の案件を get → null / not found
- [ ] テストケース: テナント A の list 操作でテナント B のデータが含まれない
- [ ] usecase / repository のモックを使用して、organizationId が一貫して渡されることを検証する
...
- [ ] `src/__tests__/mcp/mcpApproval.test.ts` を作成する
...
- [ ] `src/__tests__/mcp/mcpAuditLog.test.ts` を作成する
...
- [ ] `bun test` が green（既存テスト無変更で green + 新規テスト green）
- [ ] `bunx aozu check` が exit 0
- [ ] architecture test が green（`mod-mcp` 宣言込み）
