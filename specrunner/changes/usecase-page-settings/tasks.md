...
- [x] `src/application/usecases/listAuditLogs.ts` を作成する
...
- [x] `src/application/usecases/index.ts` に `export { listAuditLogs } from "./listAuditLogs";` を追加する
...
- [x] `src/application/usecases/listApprovalTemplates.ts` を作成する
...
- [x] `src/application/usecases/index.ts` に `export { listApprovalTemplates } from "./listApprovalTemplates";` を追加する
...
- [x] `src/application/usecases/getApprovalTemplate.ts` を作成する
  - `approvalTemplateRepository` を `@/infrastructure/repositories` から import する
  - `ApprovalTemplate` 型を `@/domain/models/approvalTemplate` から import する
...
- [x] `src/application/usecases/index.ts` に `export { getApprovalTemplate } from "./getApprovalTemplate";` を追加する
...
- [x] `src/application/usecases/getApprovalPolicy.ts` を作成する
...
- [x] `src/application/usecases/index.ts` に `export { getApprovalPolicy } from "./getApprovalPolicy";` を追加する
...
- [x] `src/app/(dashboard)/settings/audit-logs/page.tsx` を編集する
...
- [x] `src/app/(dashboard)/settings/policies/page.tsx` を編集する
...
- [x] `src/app/(dashboard)/settings/policies/new/page.tsx` を編集する
  - `import { approvalTemplateRepository } from "@/infrastructure/repositories";` を削除する
  - `import { listApprovalTemplates } from "@/application/usecases";` を追加する
  - `approvalTemplateRepository.findByOrganization(session.user.organizationId)` を `listApprovalTemplates({ organizationId: session.user.organizationId })` に置き換える
...
- [x] `src/app/(dashboard)/settings/policies/[id]/edit/page.tsx` を編集する
...
- [x] `src/app/(dashboard)/settings/templates/[id]/edit/page.tsx` を編集する
  - `import { approvalTemplateRepository } from "@/infrastructure/repositories";` を削除する
...
- [x] `bun run test` を実行して全テストが pass することを確認する
- [x] 設定系 page.tsx 全ファイルで `@/infrastructure/repositories` の grep 結果が 0 件であることを確認する
