...
- [ ] `BTN_PRIMARY` を `"bg-primary text-white text-xs font-medium rounded px-4 py-1.5 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"` に変更する
- [ ] `BTN_SECONDARY` を `"bg-bg-surface text-text text-xs font-medium rounded px-4 py-1.5 border border-border hover:bg-bg-surface-alt disabled:opacity-50"` に変更する
- [ ] `BTN_DANGER` を `"bg-danger text-white text-xs font-medium rounded px-4 py-1.5 hover:opacity-90 disabled:opacity-50"` に変更する
- [ ] `BTN_PRIMARY_DISABLED` export を削除する
- [ ] `BTN_SUCCESS` export を削除する
- [ ] `BTN_WARNING` export を削除する
- [ ] `BTN_SUBMIT` export を削除する
- [ ] `INPUT_BASE` を `"w-full border border-border rounded px-2.5 py-1.5 text-xs text-text bg-bg-surface focus:border-primary focus:outline-none placeholder:text-text-placeholder"` に更新する（`text-text bg-bg-surface placeholder:text-text-placeholder` を追加）
- [ ] `SELECT_BASE` を `"block w-full border border-border rounded px-2.5 py-1.5 text-xs text-text bg-bg-surface focus:border-primary focus:outline-none"` に更新する（`text-text bg-bg-surface` を追加）
...
- [ ] th の className を `text-table-head text-text font-medium` から `text-table-head text-text-secondary font-medium` に変更する（`text-text` → `text-text-secondary`）
- [ ] クリック可能行の hover クラスを `hover:bg-primary/10` から `hover:bg-bg-surface-alt` に変更する
...
- [ ] プログレスバーのトラック: `bg-gray-200` → `bg-border`
- [ ] 入金済セグメント: `bg-green-500` → `bg-success`（2 箇所: バーと凡例ドット）
- [ ] 請求済セグメント: `bg-blue-500` → `bg-primary`（2 箇所: バーと凡例ドット）
- [ ] 凡例の残り部分: `bg-gray-200 border border-gray-300` → `bg-border border border-border`
...
- [ ] 「受注にする」ボタンの className: `border-green-600 text-green-700 hover:bg-green-50` → `border-status-green-text text-status-green-text hover:bg-status-green-bg`（その他のクラスは維持）
- [ ] 「失注にする」ボタンの className の `hover:bg-red-50` → `hover:bg-status-red-bg`（`border-danger text-danger` は既にトークン参照のため変更不要）
- [ ] 「見送りにする」ボタンの className: `border-gray-500 text-gray-600 hover:bg-gray-50` → `border-status-gray-text text-status-gray-text hover:bg-status-gray-bg`
...
- [ ] 却下ボタンの className の `bg-white` → `bg-bg-surface`
- [ ] 却下ボタンの className の `hover:bg-red-50` → `hover:bg-status-red-bg`
...
- [ ] `src/app/(dashboard)/clients/[id]/ClientInfoSection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
- [ ] `src/app/(dashboard)/inquiries/[id]/InquiryCustomerSection.tsx`
...
- [ ] `src/app/(dashboard)/inquiries/[id]/InquiryActions.tsx`
...
- [ ] `src/app/(dashboard)/inquiries/[id]/InquiryInfoSection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
- [ ] `src/app/(dashboard)/contracts/[id]/ContractInfoSection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
...
- [ ] `src/app/(dashboard)/contracts/[id]/invoices/[invoiceId]/InvoiceActions.tsx`
...
- [ ] `src/app/(dashboard)/deals/[id]/DealNotesSection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
- [ ] `src/app/(dashboard)/deals/[id]/DealInfoSection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
- [ ] `src/app/(dashboard)/deals/[id]/meetings/[meetingId]/MeetingActionItemsSection.tsx`
  - 両方の `bg-green-600 text-white` → `bg-primary text-white hover:opacity-90`
- [ ] `src/app/(dashboard)/deals/[id]/meetings/[meetingId]/MeetingSummarySection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
- [ ] `src/app/(dashboard)/deals/[id]/meetings/[meetingId]/MeetingHearingSection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
- [ ] `src/app/(dashboard)/deals/[id]/meetings/[meetingId]/MeetingInfoSection.tsx`
...
- [ ] `src/app/(dashboard)/deals/[id]/meetings/[meetingId]/MeetingAttendeesSection.tsx`
  - `"bg-green-600 text-white cursor-pointer"` → `"bg-primary text-white cursor-pointer hover:opacity-90"`
...
- [ ] `src/app/(dashboard)/settings/organization/OrganizationForm.tsx`
...
- [ ] `src/app/(dashboard)/settings/policies/PolicyForm.tsx`
  - error: 同上
...
- [ ] `src/app/(dashboard)/account/ProfileForm.tsx`
  - success/error: 同パターン
- [ ] `src/app/(dashboard)/account/PasswordForm.tsx`
  - success/error: 同パターン
- [ ] `src/app/(dashboard)/account/ApiTokenSection.tsx`
  - error: 同パターン
...
- [ ] `bg-blue-50` → `bg-bg-info`
- [ ] `border-blue-200` → `border-status-blue-text/30`
- [ ] `text-blue-500`（アイコン）→ `text-status-blue-text`
- [ ] `text-blue-800`（テキスト）→ `text-status-blue-text`

### 9-2: contracts/[id]/page.tsx（amber 警告バナー）

`src/app/(dashboard)/contracts/[id]/page.tsx` を編集する。

- [ ] `bg-amber-50` → `bg-bg-row-pending`
- [ ] `border-amber-300` → `border-border-row-pending`
...
- [ ] `isExpiringWithin30Days(row) ? "bg-amber-50" : undefined` → `isExpiringWithin30Days(row) ? "bg-bg-row-pending" : undefined`

### 9-4: NotificationPanel バッジ

`src/app/(dashboard)/NotificationPanel.tsx` を編集する。

- [ ] `bg-red-500 text-white` → `bg-danger text-white`
...
- [ ] `src/app/(dashboard)/contracts/new/NewContractForm.tsx`
...
- [ ] `src/app/(dashboard)/contracts/[id]/invoices/new/NewInvoiceForm.tsx`
  - 同様に inline submit → `SubmitButton` に置換
  - キャンセルボタン → `BTN_SECONDARY`
- [ ] `src/app/(dashboard)/deals/new/NewDealForm.tsx`
  - 同様
...
- [ ] モーダル「作成」ボタン（`bg-primary text-white text-xs px-3 py-1.5 cursor-pointer disabled:opacity-50`）→ `BTN_PRIMARY` を import して `className={BTN_PRIMARY}`
- [ ] モーダル「キャンセル」ボタン（`border border-border text-text text-xs px-3 py-1.5 cursor-pointer disabled:opacity-50`）→ `BTN_SECONDARY` を import して `className={BTN_SECONDARY}`
- [ ] 「新規作成」トップバーボタン（`text-xs font-medium px-3 py-1.5 bg-primary text-white rounded cursor-pointer`）→ `BTN_PRIMARY` を import して `className={BTN_PRIMARY}`
- [ ] select 要素の className `w-full text-xs border border-border rounded px-2 py-1.5 bg-bg-surface text-text` → `SELECT_BASE` を import して `className={SELECT_BASE}`

### 10-3: その他のインラインボタンのトークン化

- [ ] `src/app/(dashboard)/settings/audit-logs/AuditLogFilter.tsx`
  - `className="bg-primary text-white text-xs px-3 py-1.5 rounded font-medium"` → `BTN_PRIMARY` を import して `className={BTN_PRIMARY}`
- [ ] `src/app/(dashboard)/contracts/[id]/ContractInteractionSection.tsx`
...
- [ ] `src/app/(dashboard)/contracts/[id]/invoices/[invoiceId]/InvoiceInteractionSection.tsx`
  - 同様
- [ ] `src/app/(dashboard)/contracts/[id]/invoices/[invoiceId]/InvoiceActions.tsx`
  - `className="text-xs px-3 py-1.5 bg-primary text-white cursor-pointer disabled:opacity-50"` → `BTN_PRIMARY`
- [ ] `src/app/(dashboard)/contracts/[id]/InvoiceSection.tsx`
  - `className="text-xs px-3 py-1 bg-primary text-white"` → `BTN_PRIMARY`
- [ ] `src/app/(dashboard)/clients/[id]/ClientContactsSection.tsx`
...
- [ ] `src/app/(dashboard)/deals/[id]/DealContactsSection.tsx`
...
- [ ] `src/app/(dashboard)/lib/` ディレクトリを作成する
- [ ] `src/app/(dashboard)/lib/dueDateClass.ts` を作成する:
...
- [ ] `src/__tests__/dashboard/dueDateClass.test.ts` を作成する
- [ ] 以下のテストケースを含める:
...
- [ ] `dueDateClass` を import する: `import { dueDateClass } from "@/app/(dashboard)/lib/dueDateClass";`
- [ ] `showSource=true`（グリッド行）モードの期日セル（現在 `<span className="text-text-muted font-mono">`）を以下に変更する:
  ```tsx
...
- [ ] `showSource=false`（カード行）モードの期日表示（現在 `<span className="font-mono whitespace-nowrap">`）も同様に dueDateClass を適用する
- [ ] 既存の `formatDueDate` 関数はそのまま維持する

### 12-2: 表示テスト

- [ ] `src/__tests__/dashboard/ActionItemRow.test.tsx` を作成する（または既存ファイルに追記する）
- [ ] 以下のテストケースを含める（`@testing-library/react` を使用）:
  - `item.dueDate` が昨日の Date → 期日テキストのクラスに `text-danger` が含まれる
  - `item.dueDate` が今日の Date → 期日テキストのクラスに `text-warning` が含まれる
  - `item.dueDate` が null → 強調クラスなし（`text-danger` / `text-warning` が含まれない）
  - テスト内で日付を固定するために `dueDateClass` の `now` 引数を使えるよう、ActionItemRow から `dueDateClass` を呼ぶ際にも `now` を外部から注入できるよう設計すること（props に `_testNow?: Date` を追加するか、vi.useFakeTimers を使う等、実装者が適切な方法を選択する）
- [ ] mock.module 汚染を防ぐため、`afterAll` で restore する
