...
案件詳細ページ SHALL 全商談の未完了アクションアイテムを集約表示するセクションを含む MUST。

#### Scenario: 未完了アクションアイテムの集約表示

**Given** 案件に3つの商談があり、それぞれに未完了アクションアイテムがある
**When** 案件詳細ページを表示する
**Then** 全商談の未完了アクションアイテムが一覧表示され、各アイテムに商談名（種別 + 日付）が表示される
