...
- [ ] `inquiries` テーブル定義を第 3 引数のコールバック形式に変更し、`index("inquiries_org_created_at_idx").on(table.organizationId, table.createdAt)` を追加
- [ ] `deals` テーブル定義を第 3 引数のコールバック形式に変更し、`index("deals_org_created_at_idx").on(table.organizationId, table.createdAt)` を追加
- [ ] `clients` テーブル定義を第 3 引数のコールバック形式に変更し、`index("clients_org_created_at_idx").on(table.organizationId, table.createdAt)` を追加
...
- [ ] `index("contracts_org_deal_id_idx").on(table.organizationId, table.dealId)` を追加（`findAllByDealId` 用）
- [ ] `index("contracts_org_client_id_idx").on(table.organizationId, table.clientId)` を追加（`findAllByClientId` 用）
...
- [ ] `invoices` テーブル定義を第 3 引数のコールバック形式に変更し、`index("invoices_org_contract_id_idx").on(table.organizationId, table.contractId)` を追加（`findAllByContract` / `sumAmountByContract` 用）
...
- [ ] `index("meetings_org_deal_id_idx").on(table.organizationId, table.dealId)` を追加（`findAllByDeal` 用）
- [ ] `index("meetings_org_inquiry_id_idx").on(table.organizationId, table.inquiryId)` を追加（`findAllByInquiry` 用）
...
- [ ] `approvalSteps` テーブル定義を第 3 引数のコールバック形式に変更し、`index("approval_steps_request_id_idx").on(table.requestId)` を追加
...
- [ ] `clientContacts` テーブル定義を第 3 引数のコールバック形式に変更し、`index("client_contacts_client_id_idx").on(table.clientId)` を追加
...
- [ ] `index("requests_org_trigger_entity_id_idx").on(table.organizationId, table.originTriggerEntityId)` を追加（`findByOriginTriggerEntity` / `existsPendingByTriggerEntityId` 用）
...
- [ ] `webhookDeliveries` テーブル定義を第 3 引数のコールバック形式に変更し、`index("webhook_deliveries_endpoint_id_idx").on(table.endpointId)` を追加（`findByEndpointId` / `findLatestByEndpointIds` 用）
...
- [ ] `revenueTargets` テーブル定義を第 3 引数のコールバック形式に変更し、`index("revenue_targets_org_period_start_idx").on(table.organizationId, table.periodStart)` を追加
...
- [ ] `bun run db:generate` を実行し、差分マイグレーション SQL を 1 本生成する
- [ ] 生成された SQL ファイルが `CREATE INDEX` 文のみを含むことを確認する（`CREATE TABLE` / `ALTER TABLE ... ADD COLUMN` / `DROP` / `INSERT` / `UPDATE` / `DELETE` が含まれていないこと）
- [ ] 生成されたインデックスが 13 本であることを確認する
- [ ] `bunx drizzle-kit check` を実行し、journal / snapshot 整合が正常であることを確認する
- [ ] `bun run typecheck` が成功することを確認する
- [ ] `bun run build` が成功することを確認する
- [ ] `bun test` が既存テスト無変更で成功することを確認する
