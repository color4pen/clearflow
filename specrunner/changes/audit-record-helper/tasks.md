...
- [ ] `src/application/services/auditRecorder.ts` を新規作成する
- [ ] ジェネリック関数 `recordAudit<A extends AuditAction>` を定義する
...
- [ ] 内部では `auditLogRepository.create` を純粋に委譲呼び出しする（追加ロジックなし）
- [ ] `AuditRecordParams` 型をファイルからエクスポートする（型テストで使用）
...
- [ ] `src/__tests__/static/auditRecorder.test.ts` を新規作成する
- [ ] テスト 1: `recordAudit` が `AuditAction` と `AuditTargetType` を型に持つことを静的検証する（ソースの型アノテーション文字列を検査）
- [ ] テスト 2: `AuditRecordParams` が `action_item.toggle` の metadata を `{ done: boolean }` として要求することを型テスト（`@ts-expect-error` または `tsd` の `expectType` 相当）で検証する
- [ ] テスト 3: `auditLogRepository.create` の直接呼び出しがヘルパ実装以外の `src/application/` および `src/infrastructure/handlers/` に残っていないことを検証する
...
- [ ] 全 `await auditLogRepository.create({ ... }, tx)` を `await recordAudit({ ... }, tx)` に置換する
- [ ] tx 引き回しパターンを維持する（`tx` が渡されている呼び出しは `recordAudit` の第 2 引数に `tx` を引き継ぐ）
...
- [ ] `src/infrastructure/handlers/auditLogHandler.ts` の `auditLogRepository.create` を `recordAudit` に置換する
- [ ] import 文を `recordAudit` に変更し、`auditLogRepository` の import を削除する
...
- [ ] 以下の各テストファイルで `auditLogRepository.create` を検査しているアサーションを `recordAudit` に更新する:
...
- [ ] `tsc --noEmit` が green であることを確認する
- [ ] `bun run lint` が green であることを確認する
- [ ] `bun test` が全件 green であることを確認する
- [ ] T-02 のガードテストが green であることを再確認する（全移行完了後）
