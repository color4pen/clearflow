...
┌─────────────────────────────────────────────────────┐
│                      営業管理                          │
│  引合(Inquiry) ─→ 案件(Deal) ─→ 顧客接点(Interaction)  │
└──────────────────────────┬──────────────────────────┘
                         │ 受注
┌────────────────────────▼────────────────────────┐
│               契約・請求管理                       │
...
本システムは契約・請求に関する**やり取りの記録**（関連先 relatedTo が契約/請求 の顧客接点）までを扱い、次の領域は作らない。

- **消込**: 入金と請求の突合。
- **督促**: 支払遅延に対する自動通知・督促状の発行。
- **回収**: 売掛金・債権の管理。

これらの売掛金・債権管理（AR）は別システムの領域とする。請求ステータス（scheduled / invoiced / paid / overdue）の記録は既存どおり行う。
...
### 4.3 顧客接点 (Interaction)

顧客との接点の記録。`kind`（種別＝チャネル: meeting / call / email / note）で接触手段を表し、文脈は関連先 relatedTo（案件・引合・契約・請求・顧客のいずれか）で表す。接点が案件/引合に紐づくときは商談（営業文脈）であり、引合フェーズのヒアリングから案件フェーズの提案・交渉まで一連の営業活動を時系列で記録する。接点が契約/請求に紐づくときは契約調整・請求調整となる。既存 Meeting を一般化したもので、UI/日本語表記は「商談」を維持する（ユビキタス言語辞書を参照）。
...
| id | ID | Yes | |
| organizationId | ID | Yes | テナント識別子 |
| kind | InteractionKind | Yes | 接点種別＝チャネル（接触手段）。meeting = 会議 / call = 電話 / email = メール / note = メモ。文脈（商談・契約調整・請求調整）は kind ではなく関連先 relatedTo で表す |
| dealId / inquiryId / contractId / invoiceId / clientId | ID | No | 関連先 relatedTo（いずれか 1 つ以上）。文脈を表す: 案件/引合 = 商談、契約 = 契約調整、請求 = 請求調整 |
| meetingType | MeetingType | No | 営業ステージ（関連先が案件/引合 の商談文脈の属性。kind とは独立） |
| date | datetime | Yes | 実施日時 |
| summary | string | No | 要旨（Markdown） |
| location | string | No | 場所 |
| details | JSON | No | 文脈固有情報（商談のヒアリング情報 HearingData など。関連先が契約/請求 の接点では調整内容の要約・メモを保持する） |
...
商談文脈（関連先 relatedTo が案件/引合）の営業ステージを表す属性。kind（チャネル）とは独立し、チャネルが会議・電話・メールのいずれであっても設定しうる。
...
**注**: アクションアイテムは独立エンティティとして切り出し済み（4.9 参照）。旧 `meetings.action_items`（JSON 埋め込み）から action_items テーブルに移行された。

**値オブジェクト: Attendee**

顧客接点の参加者。社内ユーザーまたは外部参加者を記録する。
...
- 関連先 relatedTo（dealId / inquiryId / contractId / invoiceId / clientId）の少なくとも 1 つは設定されていなければならない。
- `meetingType` と、`details` のうちヒアリング情報（HearingData）は商談文脈（関連先が案件/引合）のときにのみ意味を持つ（`details` 自体は関連先が契約/請求 の接点でも調整内容の要約・メモを保持する）。kind（チャネル）と関連先の組み合わせを縛る不変条件は設けない。
...
| interactionId | ID | No | 顧客接点（商談、紐づく場合） |
...
- interactionId, dealId, inquiryId は全て nullable。どれにも紐づかない個人タスクも許可する。
- 商談（関連先 relatedTo が案件/引合 の顧客接点）から作成した場合は interactionId と dealId の両方が設定される。
- 案件から直接作成した場合は dealId のみが設定される。

### 4.10 ウォッチ (Watch)

ユーザーが特定の案件を購読し、その案件配下の更新を通知として受け取るための登録。ユーザーと案件の組に対して 1 つ。
...
| id | ID | Yes | |
| organizationId | ID | Yes | テナント識別子 |
| userId | ID | Yes | 購読するユーザー |
| dealId | ID | Yes | 対象の案件 |
| createdAt | datetime | Yes | |

**不変条件**

- 同一ユーザー・同一案件のウォッチは 1 つだけ（重複登録不可）。
- ウォッチ対象の案件は同一テナントに属していなければならない。
...
```
        ...
        └──────────┘ └────┬─────┘
         ...
         ┌──────────────┐
         │ 顧客接点       │
         │(Interaction) │←── deal / inquiry / contract / invoice / client を参照
         └──────────────┘
```
...
| Interaction | Deal / Inquiry / Contract / Invoice / Client | 多対1 | 任意・少なくとも 1 つ必須（CHECK） |
| Contract | Deal | 多対1 | 必須。案件の phase が won の場合のみ作成可能 |
| Invoice | Contract | 多対1 | 必須 |
| Watch | User | 多対1 | 必須 |
| Watch | Deal | 多対1 | 必須。同一ユーザー・同一案件で一意 |
...
業務横断で発生する状態変更の記録（監査ログ）と、それを読み取り時に構成する派生情報（案件アクティビティ・通知）をまとめる。いずれもドメインイベント（§6）とは別系統である。

### 7.1 監査ログ (AuditLog)

各ドメインの状態変更を追記専用で記録する横断的な記録。ユースケースの処理内で `recordAudit` アプリケーションサービスを通じて同期的に記録する（ドメインイベントの非同期ハンドラ経由ではない）。両者の区別は「アーキテクチャ設計」の「ドメインイベントと監査ログの区別」を参照。この記録は案件アクティビティの表示や通知の導出に読み取られる。
...
| id | ID | Yes | |
| organizationId | ID | Yes | テナント識別子 |
| action | AuditAction | Yes | 操作の種類（`<対象>.<操作>` 形式） |
| targetType | AuditTargetType | Yes | 操作対象のエンティティ種別 |
| targetId | ID | Yes | 操作対象の識別子 |
| actorId | ID | Yes | 操作した社内ユーザー |
| metadata | JSON | No | 操作固有の補足情報 |
| createdAt | datetime | Yes | 記録日時 |

**操作カタログ (AuditAction / AuditTargetType)**

記録しうる操作と対象は型カタログとして一元定義する。正本は `src/domain/models/auditLog.ts`。

- `AuditAction`: 各ドメインの作成・更新・削除・状態遷移を表す `<対象>.<操作>` 形式の列挙（例: `deal.create` / `deal.updatePhase` / `contract.updateStatus` / `invoice.update_status` / `request.approve` / `action_item.toggle`）。
- `AuditTargetType`: 操作対象のエンティティ種別（例: `deal` / `interaction` / `meeting` / `contract` / `invoice` / `action_item` / `request` / `client`）。`interaction` は顧客接点（商談を含む）を表し、既存 `meeting` を一般化したもの。
- `AuditMetadataMap`: 特定アクションに付随する型付きメタデータの対応。現状の正本には `action_item.toggle`（→ `{ done: boolean }`）と `action_item.updateStatus` の 2 件のみ登録されている。状態遷移アクション（`deal.updatePhase`・`contract.updateStatus`・`invoice.update_status`）の変更前後（from/to）は未登録であり、タイムラインの遷移表示を実装する際に該当エントリを追加する。

**不変条件**

- 追記専用。記録後の更新・削除はしない。
- すべての記録は `organizationId` でテナント分離される。

### 7.2 案件アクティビティ (Deal Activity)

案件配下で発生した**顧客接点（Interaction）と業務イベント（Event）**を、監査ログから**厳選・集約**して新しい順に並べたタイムライン（UI 呼称「アクティビティ」）。永続化された独自エンティティは持たない。監査ログを無選別に流すのではなく、業務的に意味のあるものだけを表示する（用語は「ユビキタス言語辞書」の「タイムラインの構成概念」を参照）。

**読み取りモデル: DealActivityResult**

| 要素 | 説明 |
|---|---|
| logs | 表示対象に厳選・集約した監査ログ（新しい順、表示件数に上限あり） |
| targetInfoMap | 各操作対象の表示名と詳細リンク（`<targetType>:<targetId>` をキーとする） |

**表示対象アクションの分類**

| 分類 | 表示 | 対象アクション |
|---|---|---|
| 顧客接点（Interaction） | 表示 | `interaction.create`（関連先 relatedTo を問わず顧客接点を記録。商談に加え、関連先が契約/請求 の契約調整・請求調整も含む）。既存 `meeting.create` も同分類で扱う |
| 業務イベント（Event） | 表示 | `deal.create`（案件作成） / `deal.updatePhase`（フェーズ変更・受注・失注） / `contract.create`（契約作成） / `contract.updateStatus`（契約ステータス変更） / `invoice.create`（請求作成） / `invoice.update_status`（請求ステータス変更） |
| 除外（監査ログのみ・非表示） | 非表示 | 細かなフィールド更新（`deal.update` / `contract.update` / `invoice.update` / `interaction.update`（kind を問わず。表示行の `interaction.create` と対称）。既存 `meeting.update` も同分類で扱う）、タスク（`action_item.*`＝別概念）、案件担当者の増減（`deal_contact.*`） |

- **対象範囲**: 案件本体と、その配下の商談・契約・請求。請求は契約経由（`invoice.contractId`）で解決する。タスク（`action_item`）・案件担当者（`deal_contact`）・承認リクエストはタイムライン対象とせず、取得対象からも外す。
- **状態遷移の表示**: フェーズ変更・契約ステータス変更は metadata の遷移情報（`fromPhase`/`toPhase`・`fromStatus`/`toStatus`）から「変更前 → 変更後」を表示する。`invoice.update_status` も遷移情報を metadata に記録する（記録のない既存ログは遷移を表示しない）。型安全に扱うため `AuditMetadataMap`（§7.1）に該当アクションのエントリを加える。
- **集約と件数上限**: 同一の `(操作者・アクション・対象)` が連続する場合は 1 件にまとめ（件数を示す）、状態遷移系が連続する場合は最初の変更前から最後の変更後への正味の遷移にまとめる。集約は表示用の読み取り処理であり、監査ログ自体は欠落させない。取得 → 厳選・集約 → 件数上限（`ACTIVITY_TIMELINE_LIMIT`）の順に適用し、取得時点で上限により切り落とさない。
- 表示名・リンクは既に取得済みのエンティティから構成し、追加のリポジトリ取得は行わない。
- 表示対象アクションはコードの固定分類で決める。`ACTIVITY_HIDDEN_ACTIONS`（環境変数）は固定分類に対し追加で非表示にする補助設定とする。

### 7.3 通知 (Notification)

ユーザーがウォッチしている案件配下の更新を、監査ログから読み取って導出する読み取りモデル。永続化された通知レコードは持たない。

**読み取りモデル: DerivedNotification**

| 要素 | 説明 |
|---|---|
| log | 通知の元になった監査ログ |
| dealId / dealTitle | 起点となるウォッチ対象の案件 |
| targetInfo | 操作対象の表示名と詳細リンク |
| isUnread | ユーザーの最終確認時刻より新しいか |

- 通知対象とする操作は限定する（`NOTIFICATION_ACTIONS`: `deal.update` / `deal.updatePhase` / `interaction.create`（商談＝関連先 relatedTo が案件/引合 の接点。既存 `meeting.create` と併記） / `action_item.create` / `contract.create`）。
- 自分自身の操作は除外する。
- 未読は `users.notificationsLastSeenAt` を基準に判定し、既読化はこの時刻を更新する。
...
| 顧客接点 | Interaction | 顧客との接点の記録。kind（チャネル: meeting / call / email / note）で接触手段を、関連先 relatedTo（案件・引合・契約・請求・顧客）で文脈を表す |
| 商談 | Interaction (relatedTo = 案件/引合) | 営業活動における打合せ。顧客接点のうち関連先が案件または引合のもの（営業文脈）。チャネルは会議が主だが電話・メールもありうる。UI/日本語表記は「商談」を用いる |
| 顧客 | Client | 取引先企業 |
