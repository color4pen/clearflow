...
本システムは契約・請求に関する**やり取りの記録**（顧客接点の `contract_adjustment` / `invoice_adjustment`）までを扱い、次の領域は作らない。
...
顧客との接点（商談・電話・メール・契約調整・請求調整など）の記録。商談は `kind = meeting` の一種で、引合フェーズのヒアリングから案件フェーズの提案・交渉まで一連の営業活動を時系列で記録する。接点は関連先（案件・引合・契約・請求・顧客）のいずれかに紐づく。既存 Meeting を一般化したもので、UI/日本語表記は「商談」を維持する（ユビキタス言語辞書を参照）。
...
| id | ID | Yes | |
| organizationId | ID | Yes | テナント識別子 |
| kind | InteractionKind | Yes | 接点種別。meeting = 商談 / call = 電話 / email = メール / contract_adjustment = 契約調整（契約内容の見直し・変更交渉などのやり取り） / invoice_adjustment = 請求調整（請求内容の修正・調整交渉などのやり取り） |
| dealId / inquiryId / contractId / invoiceId / clientId | ID | No | 関連先（いずれか 1 つ以上） |
| meetingType | MeetingType | No | 商談種別（kind = meeting のとき） |
| date | datetime | Yes | 実施日時 |
| summary | string | No | 要旨（Markdown） |
| location | string | No | 場所 |
| details | JSON | No | kind 固有情報（商談のヒアリング情報 HearingData など。contract_adjustment / invoice_adjustment では調整内容の要約・メモを保持する） |
...
- 関連先（dealId / inquiryId / contractId / invoiceId / clientId）の少なくとも 1 つは設定されていなければならない。
- `kind = contract_adjustment` のとき `contractId`、`kind = invoice_adjustment` のとき `invoiceId` が設定されていなければならない。
- `meetingType` と、`details` のうちヒアリング情報（HearingData）は `kind = meeting` のときにのみ意味を持つ（`details` 自体は contract_adjustment / invoice_adjustment でも調整内容の要約・メモを保持する）。
...
- 商談（kind = meeting の顧客接点）から作成した場合は interactionId と dealId の両方が設定される。
...
| 分類 | 表示 | 対象アクション |
|---|---|---|
| 顧客接点（Interaction） | 表示 | `interaction.create`（kind を問わず顧客接点を記録。商談に加え契約調整・請求調整も含む）。既存 `meeting.create` も同分類で扱う |
...
- 通知対象とする操作は限定する（`NOTIFICATION_ACTIONS`: `deal.update` / `deal.updatePhase` / `interaction.create`(kind = meeting)（既存 `meeting.create` と併記） / `action_item.create` / `contract.create`）。
- 自分自身の操作は除外する。
...
| 顧客接点 | Interaction | 顧客との接点の記録（商談・電話・メール・契約調整・請求調整など）。商談は kind = meeting の一種 |
| 商談 | Interaction (kind = meeting) | 営業活動における打合せ。顧客接点のうち kind = meeting のもの。UI/日本語表記は「商談」を用いる |
| 顧客 | Client | 取引先企業 |
