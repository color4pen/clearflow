...
業務横断で発生する状態変更の記録（監査ログ）と、それを読み取り時に構成する派生情報（案件アクティビティ等）をまとめる。いずれもドメインイベント（§6）とは別系統である。

### 7.1 監査ログ (AuditLog)

各ドメインの状態変更を追記専用で記録する横断的な記録。ユースケースの処理内で `recordAudit` アプリケーションサービスを通じて同期的に記録する（ドメインイベントの非同期ハンドラ経由ではない）。両者の区別は「アーキテクチャ設計」の「ドメインイベントと監査ログの区別」を参照。この記録は案件アクティビティの表示などに読み取られる。
