...
- `AuditTargetType`: 操作対象のエンティティ種別（例: `deal` / `meeting` / `contract` / `invoice` / `action_item` / `request` / `client`）。
- `AuditMetadataMap`: 特定アクションに付随する型付きメタデータの対応（例: `action_item.toggle` → `{ done: boolean }`）。
...
案件配下のエンティティ（案件・商談・契約・請求・アクションアイテム・案件担当者）に対する操作を、監査ログから読み取って新しい順に並べた読み取りモデル。永続化された独自エンティティは持たない。

**読み取りモデル: DealActivityResult**

| 要素 | 説明 |
|---|---|
| logs | 対象エンティティ群の監査ログ（新しい順、表示件数に上限あり） |
| targetInfoMap | 各操作対象の表示名と詳細リンク（`<targetType>:<targetId>` をキーとする） |

- 対象は案件本体に加え、その配下の商談・契約・請求・アクションアイテム・案件担当者。請求は契約経由（`invoice.contractId`）で解決する。
- 表示名・リンクは既に取得済みのエンティティから構成し、追加のリポジトリ取得は行わない。案件担当者は氏名解決をしないため表示名マップに含めない。
- 表示件数の上限と除外アクションは設定で制御する（`ACTIVITY_TIMELINE_LIMIT`）。
...
- 通知対象とする操作は限定する（`NOTIFICATION_ACTIONS`: `deal.update` / `deal.updatePhase` / `meeting.create` / `action_item.create` / `contract.create`）。
- 自分自身の操作は除外する。
