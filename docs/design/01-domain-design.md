...
各ドメインの状態変更を追記専用で記録する横断的な記録。ユースケースの処理内で `recordAudit` アプリケーションサービスを通じて同期的に記録する（ドメインイベントの非同期ハンドラ経由ではない）。ドメインイベント（§6）とは別系統であり、両者の区別は「アーキテクチャ設計」の「ドメインイベントと監査ログの区別」を参照。この記録は案件アクティビティの表示や通知の導出に読み取られる。
