 ...
 * T-06: interactions update_meeting の internalAttendees / externalAttendees が
...
describe("T-06: interactions update_meeting — attendees 内部/外部独立部分更新", () => {
  it("internalAttendees のみ指定 → usecase に internalAttendees が渡り externalAttendees は undefined", async () => {
    const result = await callInteractions({
      operation: "update_meeting",
      meetingId: MEETING_UUID,
    ...
    // externalAttendees は省略されたので undefined（反対側を保持する）
  ...
  it("両方指定 → internalAttendees と externalAttendees の両方が渡る", async () => {
    ...
    expect(Array.isArray(args.externalAttendees)).toBe(true);
    expect((args.externalAttendees as unknown[]).length).toBe(1);
  });

  it("両方省略 → internalAttendees と externalAttendees の両方が undefined", async () => {
    const result = await callInteractions({
      operation: "update_meeting",
      meetingId: MEETING_UUID,
      summary: "新サマリ",
