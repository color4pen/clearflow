...
function makeInteraction(kind: "contract_adjustment" | "invoice_adjustment"): Interaction {
  return {
    id: "interaction-001",
    organizationId: "org-001",
    kind,
    dealId: null,
    inquiryId: null,
    contractId: kind === "contract_adjustment" ? VALID_CONTRACT_ID : null,
    invoiceId: kind === "invoice_adjustment" ? VALID_INVOICE_ID : null,
    clientId: null,
    meetingType: null,
...
describe("TC-021: recordContractAdjustmentAction — 認可不足（finance ロール）", () => {
...
describe("TC-021: recordInvoiceAdjustmentAction — 認可不足（member ロール）", () => {
...
describe("TC-023: recordContractAdjustmentAction — 成功", () => {
  it("usecase を呼び出し、revalidatePath を実行し、空オブジェクトを返す", async () => {
    ...
    contractInfra.createdInteraction = makeInteraction("contract_adjustment");
...
describe("TC-023: recordInvoiceAdjustmentAction — 成功", () => {
  it("usecase を呼び出し、revalidatePath を実行し、空オブジェクトを返す", async () => {
    ...
    invoiceInfra.createdInteraction = makeInteraction("invoice_adjustment");
