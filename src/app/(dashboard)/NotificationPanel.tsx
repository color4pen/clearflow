...
export function NotificationPanel({ notifications, unreadCount, actorNames }: Props) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <>
          {/* dim オーバーレイ: 本文側のみを覆う（サイドバーは明るいまま） */}
          <div
            className="fixed top-0 bottom-0 left-[210px] right-0 z-30 bg-black/30"
            onClick={() => setIsOpen(false)}
            aria-hidden="true"
          />

          {/* flyout: サイドバー右端に密着して全高で開く */}
          <div className="fixed top-0 left-[210px] z-40 h-screen w-80 bg-white dark:bg-bg-card border-r border-border shadow-xl flex flex-col">
